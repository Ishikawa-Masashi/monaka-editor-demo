...
const ttPolicy = (window as any).trustedTypes?.createPolicy(
  ...
  {
    createScriptURL: (value: any) => value,
  }
);
...
function getWorker(workerId: string, label: string): Worker | Promise<Worker> {
  ...
  if (globals.MonacoEnvironment) {
    ...
    if (typeof globals.MonacoEnvironment.getWorkerUrl === 'function') {
      const wokerUrl = <string>(
        globals.MonacoEnvironment.getWorkerUrl(workerId, label)
      );
      return new Worker(
        ttPolicy
          ? (ttPolicy.createScriptURL(wokerUrl) as unknown as string)
          : wokerUrl,
        { name: label }
      );
    }
  ...
  if (typeof require === 'function') {
    ...
    const workerMain = (require as any).toUrl('./' + workerId); // explicitly using require.toUrl(), see https://github.com/microsoft/vscode/issues/107440#issuecomment-698982321
...
export function getWorkerBootstrapUrl(
  ...
  if (
    ...
    const workerBaseUrl = (require as any)
      .toUrl(myPath)
      .slice(0, -myPath.length); // explicitly using require.toUrl(), see https://github.com/microsoft/vscode/issues/107440#issuecomment-698982321
