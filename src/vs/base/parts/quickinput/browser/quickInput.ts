...
import {
  CountBadge,
  ICountBadgetyles,
} from '../../../../../vs/base/browser/ui/countBadge/countBadge';
import {
  ProgressBar,
  IProgressBarStyles,
} from '../../../../../vs/base/browser/ui/progressbar/progressbar';
...
class QuickPick<T extends IQuickPickItem>
  extends QuickInput
  implements IQuickPick<T>
{
  ...
  private _itemActivation = this.ui.isScreenReaderOptimized()
    ? ItemActivation.NONE /* https://github.com/microsoft/vscode/issues/57501 */
      ...
      this.visibleDisposables.add(
        this.ui.inputBox.onDidChange((value) => {
          ...
          const didFilter = this.ui.list.filter(
            this.filterValue(this.ui.inputBox.value)
          );
          if (didFilter) {
            this.trySelectFirst();
          }
  ...
  protected override update() {
    ...
    this.ui.container.classList.toggle(
      'hidden-input',
      hideInput && !this.description
    );
    ...
    const ariaLabel =
      this.ariaLabel || this.placeholder || QuickPick.DEFAULT_ARIA_LABEL;
...
export class QuickInputController extends Disposable {
  ...
  private registerKeyModsListeners() {
    ...
    this._register(
      dom.addDisposableListener(window, dom.EventType.KEY_DOWN, listener, true)
    );
    this._register(
      dom.addDisposableListener(window, dom.EventType.KEY_UP, listener, true)
    );
    this._register(
  ...
  private getUI() {
    ...
    this._register(
      dom.addStandardDisposableListener(checkAll, dom.EventType.CHANGE, (e) => {
    ...
    const extraContainer = dom.append(
      headerContainer,
    ...
    const list = this._register(
      new QuickInputList(container, this.idPrefix + 'list', this.options)
    );
    this._register(
    ...
    this._register(
      list.onDidChangeFocus(() => {
        if (this.comboboxAccessibility) {
          this.getUI().inputBox.setAttribute(
            'aria-activedescendant',
            this.getUI().list.getActiveDescendant() || ''
          );
        }
      })
    );
    ...
    this.ui = {
      ...
      setComboboxAccessibility: (enabled) =>
        this.setComboboxAccessibility(enabled),
  ...
  pick<T extends IQuickPickItem, O extends IPickOptions<T>>(
    ...
    return new Promise<R>((doResolve, reject) => {
      ...
      input.autoFocusOnList =
        options.autoFocusOnList === undefined || options.autoFocusOnList; // default to true
  ...
  private setVisibilities(visibilities: Visibilities) {
    ...
    ui.container.classList[visibilities.checkBox ? 'add' : 'remove'](
      'show-checkboxes'
    );
  ...
  private setComboboxAccessibility(enabled: boolean) {
    if (enabled !== this.comboboxAccessibility) {
      ...
      if (this.comboboxAccessibility) {
        ...
        ui.inputBox.setAttribute(
          'aria-activedescendant',
          ui.list.getActiveDescendant() || ''
        );
      } else {
  ...
  hide(reason?: QuickInputHideReason) {
    ...
    if (controller) {
      ...
      if (!focusChanged) {
        if (
          this.previousFocusElement &&
          this.previousFocusElement.offsetParent
        ) {
  ...
  private updateStyles() {
    if (this.ui) {
      ...
      this.ui.titleBar.style.backgroundColor = quickInputTitleBackground
        ? quickInputTitleBackground.toString()
        : '';
      this.ui.container.style.backgroundColor = quickInputBackground
        ? quickInputBackground.toString()
        : '';
      this.ui.container.style.color = quickInputForeground
        ? quickInputForeground.toString()
        : '';
      this.ui.container.style.border = contrastBorder
        ? `1px solid ${contrastBorder}`
        : '';
