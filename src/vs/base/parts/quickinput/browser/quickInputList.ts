...
class ListElementRenderer
  implements IListRenderer<ListElement, IListElementTemplateData>
{
    ...
    if (buttons && buttons.length) {
      data.actionBar.push(
        buttons.map((button, index) => {
          let cssClasses =
            button.iconClass ||
            (button.iconPath ? getIconClass(button.iconPath) : undefined);
...
export class QuickInputList {
  ...
  onChangedAllVisibleChecked: Event<boolean> =
    this._onChangedAllVisibleChecked.event;
  ...
  onChangedCheckedElements: Event<IQuickPickItem[]> =
    this._onChangedCheckedElements.event;
  ...
  constructor(
    ...
    this.list = options.createList(
      'QuickInput',
      this.container,
      delegate,
      [new ListElementRenderer()],
      {
    ...
    this.disposables.push(
      dom.addDisposableListener(this.container, dom.EventType.CLICK, (e) => {
        if (e.x || e.y) {
  ...
  setElements(
    inputElements: Array<IQuickPickItem | IQuickPickSeparator>
  ): void {
    ...
    this.elements = inputElements.reduce((result, item, index) => {
      if (item.type !== 'separator') {
        ...
        const saneDescription =
          item.description && item.description.replace(/\r?\n/g, ' ');
        const saneDetail = item.detail && item.detail.replace(/\r?\n/g, ' ');
        const saneAriaLabel =
          item.ariaLabel ||
          [saneLabel, saneDescription, saneDetail]
        ...
        result.push(
          new ListElement({
            ...
            checked: false,
            separator:
              previous && previous.type === 'separator' ? previous : undefined,
            fireButtonTriggered,
          })
        );
      }
    ...
    this.elementDisposables.push(
      ...this.elements.map((element) =>
        element.onChecked(() => this.fireCheckedEvents())
      )
    );
  ...
  focus(what: QuickInputListFocus): void {
    ...
    if (
      what === QuickInputListFocus.Next &&
      this.list.getFocus()[0] === this.list.length - 1
    ) {
      what = QuickInputListFocus.First;
    }

    if (
      what === QuickInputListFocus.Previous &&
      this.list.getFocus()[0] === 0
    ) {
  ...
  layout(maxHeight?: number): void {
    this.list.getHTMLElement().style.maxHeight = maxHeight
      ? `calc(${Math.floor(maxHeight / 44) * 44}px)`
      : '';
    this.list.layout();
  }
    ...
    // Filter by value (since we support icons in labels, use $(..) aware fuzzy matching)
      ...
      this.elements.forEach((element) => {
        ...
        const detailHighlights = this.matchOnDetail
          ...
          : undefined;
        const metaHighlights = this.matchOnMeta
          ? withNullAsUndefined(
              matchesFuzzyIconAware(
                query,
                parseLabelWithIcons(element.saneMeta || '')
              )
            )
          : undefined;
...
function compareEntries(
  elementA: ListElement,
  elementB: ListElement,
  lookFor: string
): number {
...
class QuickInputAccessibilityProvider
  implements IListAccessibilityProvider<ListElement>
{
