...
export function standardMouseMoveMerger(
  lastEvent: IStandardMouseMoveEventData | null,
...
export class GlobalMouseMoveMonitor<R extends { buttons: number }>
  implements IDisposable
{
  ...
  public startMonitoring(
    ...
    const listenTo: (Document | ShadowRoot)[] = windowChain.map(
    ...
    for (const element of listenTo) {
      this._hooks.add(
        dom.addDisposableThrottledListener(
          ...
          (lastEvent: R | null, currentEvent) =>
            this._mouseMoveEventMerger!(lastEvent, currentEvent as MouseEvent)
        )
      );
      this._hooks.add(
        dom.addDisposableListener(element, mouseUp, (e: MouseEvent) =>
          this.stopMonitoring(true)
        )
      );
    }
