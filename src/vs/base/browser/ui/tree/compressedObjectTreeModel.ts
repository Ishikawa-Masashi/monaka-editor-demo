...
): ITreeElement<ICompressedTreeNode<T>> {
...
function _decompress<T>(
  element: ITreeElement<ICompressedTreeNode<T>>,
  index = 0
...
function mapList<T, TFilterData>(
  ...
  return {
    splice(
      start: number,
      deleteCount: number,
      toInsert: ITreeNode<ICompressedTreeNode<T>, TFilterData>[]
    ): void {
      list.splice(
        start,
        deleteCount,
        toInsert.map((node) => nodeMapper.map(node)) as ITreeNode<
          T,
          TFilterData
        >[]
      );
