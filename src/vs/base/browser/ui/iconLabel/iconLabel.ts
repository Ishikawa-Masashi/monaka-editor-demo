...
export class IconLabel extends Disposable {
  ...
  constructor(container: HTMLElement, options?: IIconLabelCreationOptions) {
    ...
    const nameContainer = dom.append(
      this.labelContainer,
      dom.$('span.monaco-icon-name-container')
    );
    this.descriptionContainer = this._register(
      new FastLabelNode(
        dom.append(
          this.labelContainer,
  ...
  private setupHover(
    ...
    if (!this.hoverDelegate) {
      ...
      const hoverDisposable = setupCustomHover(
        this.hoverDelegate,
        htmlElement,
        tooltip
      );
  ...
  setLabel(label: string | string[], options?: IIconLabelValueOptions): void {
    ...
    if (typeof label === 'string') {
      ...
      for (let i = 0; i < label.length; i++) {
        ...
        const highlightedLabel = new HighlightedLabel(
          dom.append(this.container, name),
          this.supportIcons
        );
