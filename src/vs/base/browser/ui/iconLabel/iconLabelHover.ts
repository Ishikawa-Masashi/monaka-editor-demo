...
export function setupNativeHover(
  ...
  tooltip: string | IIconLabelMarkdownString | undefined
): void {
  if (isString(tooltip)) {
...
export function setupCustomHover(
  ...
  htmlElement: HTMLElement,
  markdownTooltip: string | IIconLabelMarkdownString | undefined
  ...
  const mouseOverDomEmitter = new DomEmitter(
    htmlElement,
  ...
  mouseOverDomEmitter.event((e: MouseEvent) => {
    ...
    const mouseLeaveDomEmitter = new DomEmitter(
      htmlElement,
      dom.EventType.MOUSE_LEAVE,
      true
    );
    mouseLeaveDomEmitter.event(mouseLeaveOrDown);
    const mouseDownDomEmitter = new DomEmitter(
      htmlElement,
    ...
    const mouseMoveDomEmitter = new DomEmitter(
      htmlElement,
    ...
    setTimeout(async () => {
      if (isHovering && tooltip) {
        ...
        if (!hoverOptions) {
          ...
          hoverDisposable = adjustXAndShowCustomHover(
            hoverOptions,
            mouseX,
            hoverDelegate,
            isHovering
          ...
          const resolvedTooltip =
            (await tooltip(tokenSource.token)) ??
            (!isString(markdownTooltip)
              ? markdownTooltip.markdownNotSupportedFallback
              : undefined);
          if (resolvedTooltip) {
            hoverOptions = {
...
function getTooltipForCustom(
  markdownTooltip: string | IIconLabelMarkdownString
): (token: CancellationToken) => Promise<string | IMarkdownString | undefined> {
...
function adjustXAndShowCustomHover(
  hoverOptions: IHoverDelegateOptions | undefined,
