      ...
      this.menuDisposables.add(
        addDisposableListener(menuElement, EventType.KEY_DOWN, (e) => {
          ...
          if (this.mnemonics.has(key)) {
            ...
            if (actions.length === 1) {
              if (
                actions[0] instanceof SubmenuMenuActionViewItem &&
                actions[0].container
              ) {
...
class BaseMenuActionViewItem extends BaseActionViewItem {
  ...
  override render(container: HTMLElement): void {
    ...
    this.check = append(
      this.item,
      $('span.menu-item-check' + menuSelectionIcon.cssSelector)
    );
      ...
      if (label) {
        ...
        if (matches) {
          ...
          if (escMatch) {
            this.label.append(
              strings.ltrim(
                replaceDoubleEscapes(label.substr(0, escMatch.index)),
                ' '
              ),
  ...
  override render(container: HTMLElement): void {
    ...
    if (this.item) {
      ...
      this.submenuIndicator = append(
        this.item,
        $('span.submenu-indicator' + menuSubmenuIcon.cssSelector)
      );
