...
export class Sash extends Disposable {
  ...
  private readonly _onDidEnablementChange = this._register(
  ...
  set orthogonalStartSash(sash: Sash | undefined) {
    ...
    if (sash) {
      ...
      this.orthogonalStartSashDisposables.add(
        sash.onDidEnablementChange(onChange, this)
      );
      onChange(sash.state);
    }
  ...
  constructor(
    ...
    layoutProvider: IVerticalSashLayoutProvider,
    options: ISashOptions
  );
  constructor(
    container: HTMLElement,
    layoutProvider: IHorizontalSashLayoutProvider,
    options: ISashOptions
  );
  constructor(
    container: HTMLElement,
    ...
    this._register(
      onMouseDown((e) => this.onPointerStart(e, new MouseEventFactory()), this)
    );
    ...
    const onDoubleTap = Event.map(
      Event.filter(
        Event.debounce<GestureEvent, { event: GestureEvent; count: number }>(
          onTap,
          (res, event) => ({ event, count: (res?.count ?? 0) + 1 }),
          250
        ),
        ({ count }) => count === 2
      ),
  ...
  private getOrthogonalSash(e: PointerEvent): Sash | undefined {
    ...
    if (e.target.classList.contains('orthogonal-drag-handle')) {
      return e.target.classList.contains('start')
