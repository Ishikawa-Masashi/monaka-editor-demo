...
export class ActionBar extends Disposable implements IActionRunner {
  ...
  private _onDidCancel = this._register(
    new Emitter<void>({
  ...
  constructor(container: HTMLElement, options: IActionBarOptions = {}) {
    ...
    this._register(
      this._actionRunner.onBeforeRun((e) => this._onBeforeRun.fire(e))
    );
    ...
    this._register(
      DOM.addDisposableListener(this.domNode, DOM.EventType.KEY_DOWN, (e) => {
        ...
        if (
          previousKeys &&
          (event.equals(previousKeys[0]) || event.equals(previousKeys[1]))
        ) {
  ...
  protected updateFocus(fromRight?: boolean, preventScroll?: boolean): void {
    ...
    for (let i = 0; i < this.viewItems.length; i++) {
      ...
      if (i === this.focusedItem) {
        ...
        if (
          this.options.focusOnlyEnabledItems &&
          types.isFunction(item.isEnabled) &&
          !item.isEnabled()
        ) {
          focusItem = false;
        }
