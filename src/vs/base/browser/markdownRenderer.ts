...
const _ttpInsane = (window as any).trustedTypes?.createPolicy('insane', {
  createHTML(value: any, options: InsaneOptions): string {
...
export function renderMarkdown(
  ...
  renderer.link = (href, title, text): string => {
    ...
    if (
      ...
      href = href
        ...
        .replace(/"/g, '&quot;')
        .replace(/'/g, '&#39;');
      return `<a href="#" data-href="${href}" title="${
        title || href
      }">${text}</a>`;
    }
  };
  ...
  if (options.codeBlockRenderer) {
    renderer.code = (code, lang) => {
      ...
      const promise = Promise.all([value, withInnerHTML])
        .then((values) => {
          const span = <HTMLDivElement>(
            element.querySelector(`div[data-code="${id}"]`)
          );
          if (span) {
  ...
  if (options.actionHandler) {
    options.actionHandler.disposeables.add(
      Event.any<MouseEvent>(
        domEvent(element, 'click'),
        domEvent(element, 'auxclick')
      )((e) => {
  ...
  if (options.asyncRenderCallback) {
    ...
    //     })
    // }
    const values = element.getElementsByTagName('img');
...
function sanitizeRenderedMarkdown(
  ...
  return (
    _ttpInsane?.createHTML(renderedMarkdown, insaneOptions) ??
    insane(renderedMarkdown, insaneOptions)
  );
}
    ...
    filter(token: {
      ...
      if (token.tag === 'span' && options.isTrusted) {
        if (token.attrs['style'] && Object.keys(token.attrs).length === 1) {
          ...
          return !!token.attrs['class'].match(
