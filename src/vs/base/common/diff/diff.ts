...
export class LcsDiff {
    ...
    for (let i = changes.length - 1; i >= 0; i--) {
      ...
      for (let delta = 1; ; delta++) {
        ...
        const touchingPreviousChange =
          originalStart === originalStop && modifiedStart === modifiedStop;
        const score =
  ...
  private _OriginalIsBoundary(index: number): boolean {
    ...
    return (
      this._hasStrings && /^\s*$/.test(this._originalStringElements[index])
    );
  }
  ...
  private _ModifiedIsBoundary(index: number): boolean {
    ...
    return (
      this._hasStrings && /^\s*$/.test(this._modifiedStringElements[index])
    );
  }
