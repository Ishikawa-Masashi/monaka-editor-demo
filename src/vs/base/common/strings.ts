...
export function nextCharLength(str: string, offset: number): number {
  ...
  offset +=
    initialCodePoint >= Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1;
  ...
  while (offset < len) {
    ...
    const nextGraphemeBreakType =
      graphemeBreakTree.getGraphemeBreakType(nextCodePoint);
    if (
      breakBetweenGraphemeBreakType(graphemeBreakType, nextGraphemeBreakType)
    ) {
      break;
    }
    offset +=
      nextCodePoint >= Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1;
...
export function prevCharLength(str: string, offset: number): number {
  ...
  while (offset > 0) {
    ...
    const prevGraphemeBreakType =
      graphemeBreakTree.getGraphemeBreakType(prevCodePoint);
    if (
      breakBetweenGraphemeBreakType(prevGraphemeBreakType, graphemeBreakType)
    ) {
      break;
    }
    offset -=
      prevCodePoint >= Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1;
...
export function breakBetweenGraphemeBreakType(
  ...
  if (
    breakTypeB === GraphemeBreakType.Extend ||
    breakTypeB === GraphemeBreakType.ZWJ
  ) {
    return false; // GB9
  }
