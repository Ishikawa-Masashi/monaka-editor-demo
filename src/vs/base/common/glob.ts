...
function trivia3(pattern: string, options: IGlobOptions): ParsedStringPattern {
  ...
  if (withBasenames) {
    parsedPattern.allBasenames = (<ParsedStringPattern>(
      withBasenames
    )).allBasenames;
  }
...
function trivia4and5(
  ...
  const nativePath = usingPosixSep
    ? targetPath
    : targetPath.replace(ALL_FORWARD_SLASHES, paths.sep);
  ...
  const parsedPattern: ParsedStringPattern = matchPathEnds
    ? function (testPath, basename) {
        return typeof testPath === 'string' &&
          (testPath === nativePath ||
            testPath.endsWith(nativePathEnd) ||
            (!usingPosixSep &&
              (testPath === targetPath || testPath.endsWith(targetPathEnd))))
          ? pattern
          : null;
      }
    : function (testPath, basename) {
...
function parsedExpression(
  ...
  if (
    ...
    if (withBasenames) {
      resultExpression.allBasenames = (<ParsedStringPattern>(
        withBasenames
      )).allBasenames;
    }
