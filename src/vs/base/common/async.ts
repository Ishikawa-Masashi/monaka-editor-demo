...
export namespace Promises {
  ...
  interface PromiseWithAllSettled<T> {
    allSettled<T>(
      promises: Promise<T>[]
    ): Promise<readonly (IResolvedPromise<T> | IRejectedPromise)[]>;
  }
  ...
  export async function allSettled<T>(
    ...
    if (
      typeof (Promise as unknown as PromiseWithAllSettled<T>).allSettled ===
      'function'
    ) {
  ...
  async function allSettledNative<T>(
    promises: Promise<T>[]
  ): Promise<readonly (IResolvedPromise<T> | IRejectedPromise)[]> {
    return (Promise as unknown as PromiseWithAllSettled<T>).allSettled(
      promises
    );
  }

  async function allSettledShim<T>(
    promises: Promise<T>[]
  ): Promise<readonly (IResolvedPromise<T> | IRejectedPromise)[]> {
    return Promise.all(
      promises.map((promise) =>
        promise.then(
          (value) => {
            const fulfilled: IResolvedPromise<T> = {
              status: 'fulfilled',
              value,
            };
          ...
          (error) => {
            const rejected: IRejectedPromise = {
              status: 'rejected',
              reason: error,
            };
