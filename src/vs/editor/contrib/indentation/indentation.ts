    ...
    if (currentLineText !== adjustedLineContent) {
      indentEdits.push(
        EditOperation.replaceMove(
          ...
          TextModel.normalizeIndentation(globalIndent, indentSize, insertSpaces)
        )
      );
    }
  } else {
  ...
  constructor(edits: TextEdit[], initialSelection: Selection) {
    ...
    for (let edit of edits) {
      if (edit.range && typeof edit.text === 'string') {
        this._edits.push(
          edit as { range: IRange; text: string; eol?: EndOfLineSequence }
        );
      }
    }
  }
...
export class AutoIndentOnPaste implements IEditorContribution {
  ...
  private trigger(range: Range): void {
    ...
    if (!/\S/.test(firstLineText.substring(0, range.startColumn - 1))) {
      let indentOfFirstLine =
        LanguageConfigurationRegistry.getGoodIndentForLine(
          autoIndent,
          model,
          model.getLanguageIdentifier().id,
          startLineNumber,
      ...
      if (indentOfFirstLine !== null) {
        ...
        if (newSpaceCnt !== oldSpaceCnt) {
          let newIndent = indentUtils.generateIndent(
            newSpaceCnt,
            tabSize,
            insertSpaces
          );
          ...
          firstLineText =
            newIndent + firstLineText.substr(oldIndentation.length);
        } else {
          let indentMetadata = LanguageConfigurationRegistry.getIndentMetadata(
            model,
            startLineNumber
          );
    ...
    if (startLineNumber !== range.endLineNumber) {
      ...
      if (indentOfSecondLine !== null) {
        let newSpaceCntOfSecondLine = indentUtils.getSpaceCnt(
          indentOfSecondLine,
          tabSize
        );
        let oldSpaceCntOfSecondLine = indentUtils.getSpaceCnt(
        ...
        if (newSpaceCntOfSecondLine !== oldSpaceCntOfSecondLine) {
          ...
          for (let i = startLineNumber + 1; i <= range.endLineNumber; i++) {
            ...
            let originalSpacesCnt = indentUtils.getSpaceCnt(
              originalIndent,
              tabSize
            );
    ...
    if (textEdits.length > 0) {
      ...
      let cmd = new AutoIndentOnPasteCommand(
        textEdits,
  ...
  private shouldIgnoreLine(model: ITextModel, lineNumber: number): boolean {
    ...
    if (tokens.getCount() > 0) {
      let firstNonWhitespaceTokenIndex =
