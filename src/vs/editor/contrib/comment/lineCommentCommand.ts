...
export class LineCommentCommand implements ICommand {
  ...
  private _attemptRemoveBlockComment(
    ...
    startToken: string,
    endToken: string
  ): IIdentifiedSingleEditOperation[] | null {
    ...
    if (
      ...
      model
        .getLineContent(startLineNumber)
        .charCodeAt(startTokenIndex + startToken.length) === CharCode.Space
    ) {
      startToken += ' ';
    }
    ...
    if (
      endTokenIndex !== -1 &&
      model.getLineContent(endLineNumber).charCodeAt(endTokenIndex - 1) ===
        CharCode.Space
    ) {
      endToken = ' ' + endToken;
      endTokenIndex -= 1;
  ...
  private static nextVisibleColumn(
    ...
    if (isTab) {
      return (
        currentVisibleColumn + (tabSize - (currentVisibleColumn % tabSize))
      );
    }
