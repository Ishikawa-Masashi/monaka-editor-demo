...
export class SnippetSession {
  ...
  static createEditsAndSnippets(
    ...
    const workspaceService = editor.invokeWithinContext((accessor) =>
      accessor.get(IWorkspaceContextService, optional)
    );
    ...
    let firstLineFirstNonWhitespace = model.getLineFirstNonWhitespaceColumn(
  ...
  _logInfo(): string {
    return `template="${
      this._template
    }", merged_templates="${this._templateMerges.join(' -> ')}"`;
  }
