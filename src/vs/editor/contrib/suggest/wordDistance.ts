...
export abstract class WordDistance {
  ...
  static async create(
    ...
    const [ranges] =
      await new BracketSelectionRangeProvider().provideSelectionRanges(model, [
        position,
      ]);
    if (ranges.length === 0) {
      return WordDistance.None;
    ...
    return new (class extends WordDistance {
      distance(anchor: IPosition, item: CompletionItem) {
        ...
        let word =
          typeof item.label === 'string' ? item.label : item.label.name;
        ...
        let bestWordRange =
          idx >= 0 ? wordLines[idx] : wordLines[Math.max(0, ~idx - 1)];
