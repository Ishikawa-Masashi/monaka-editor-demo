...
export const suggestMoreInfoIcon = registerIcon(
  'suggest-more-info',
...
const _completionItemColor = new (class ColorExtractor {
  ...
  extract(item: CompletionItem, out: string[]): boolean {
    ...
    if (typeof item.completion.documentation === 'string') {
      const match = ColorExtractor._regexRelaxed.exec(
      ...
      if (
        ...
        (match.index === 0 ||
          match.index + match[0].length ===
            item.completion.documentation.length)
      ) {
        out[0] = match[0];
        return true;
      }
  ...
  renderElement(
    ...
    if (
      ...
      data.iconContainer.classList.add(
        'suggest-icon',
        ...completionKindToCssClass(completion.kind).split(' ')
      );
    }
    ...
    if (typeof completion.label === 'string') {
      ...
      data.parametersLabel.textContent = stripNewLines(
        completion.label.parameters || ''
      );
      data.qualifierLabel.textContent = stripNewLines(
        completion.label.qualifier || ''
      );
      data.detailsLabel.textContent = stripNewLines(
        completion.label.type || ''
      );
      data.root.classList.remove('string-label');
      data.root.title = `${element.textLabel}${
        completion.label.parameters ?? ''
      }  ${completion.label.qualifier ?? ''}  ${completion.label.type ?? ''}`;
    }
