...
export const editorSuggestWidgetSelectedForeground = registerColor(
...
export const editorSuggestWidgetHighlightFocusForeground = registerColor(
...
export class SuggestWidget implements IDisposable {
  ...
  constructor(
    ...
    const details = instantiationService.createInstance(
      SuggestDetailsWidget,
      this.editor
    );
    ...
    this._disposables.add(
      attachListStyler(this._list, _themeService, {
  ...
  private _onThemeChange(theme: IColorTheme) {
    ...
    if (backgroundColor) {
      ...
      this._details.widget.domNode.style.backgroundColor =
  ...
  toggleDetailsFocus(): void {
    if (this._state === State.Details) {
      ...
      if (this._detailsBorderColor) {
        this._details.widget.domNode.style.borderColor =
...
registerThemingParticipant((theme, collector) => {
  ...
  if (matchHighlight) {
    collector.addRule(
      `.monaco-editor .suggest-widget .monaco-list .monaco-list-row .monaco-highlighted-label .highlight { color: ${matchHighlight}; }`
    );
  }

  const matchHighlightFocus = theme.getColor(
    editorSuggestWidgetHighlightFocusForeground
  );
  if (matchHighlight) {
    collector.addRule(
      `.monaco-editor .suggest-widget .monaco-list .monaco-list-row.focused .monaco-highlighted-label .highlight { color: ${matchHighlightFocus}; }`
    );
  }
  ...
  if (foreground) {
    collector.addRule(
      `.monaco-editor .suggest-widget, .monaco-editor .suggest-details { color: ${foreground}; }`
    );
  }

  const selectedForeground = theme.getColor(
    editorSuggestWidgetSelectedForeground
  );
  if (selectedForeground) {
    collector.addRule(
      `.monaco-editor .suggest-widget .monaco-list .monaco-list-row.focused { color: ${selectedForeground}; }`
    );
  }
  ...
  if (codeBackground) {
    collector.addRule(
      `.monaco-editor .suggest-details code { background-color: ${codeBackground}; }`
    );
  }
});
