...
class LineSuffix {
  ...
  delta(position: IPosition): number {
    ...
    if (this._marker) {
      ...
      return (
        this._model.getLineMaxColumn(position.lineNumber) - position.column
      );
    }
  }
}
...
export class SuggestController implements IEditorContribution {
  ...
  constructor(
    ...
    this.widget = this._toDispose.add(
      new IdleValue(() => {
        ...
        this._toDispose.add(
          widget.onDidSelect((item) => this._insertSuggestion(item, 0), this)
        );
        ...
        this._toDispose.add(
          widget.onDidFocus(({ item }) => {
            ...
            let value = true;
            if (
              this.editor.getOption(EditorOption.acceptSuggestionOnEnter) ===
                'smart' &&
            ...
            ctxHasInsertAndReplace.set(
              !Position.equals(item.editInsertEnd, item.editReplaceEnd)
            );
          ...
          widget.onDetailsKeyDown((e) => {
            ...
            if (
              e
                .toKeybinding()
                .equals(
                  new SimpleKeybinding(true, false, false, false, KeyCode.KEY_C)
                ) ||
    ...
    this._alternatives = this._toDispose.add(
      new IdleValue(() => {
        return this._toDispose.add(
          new SuggestAlternatives(this.editor, this._contextKeyService)
        );
      })
    );
    ...
    this._toDispose.add(
      this.model.onDidSuggest((e) => {
        if (!e.shy) {
          let index = this._memoryService.select(
            this.editor.getModel()!,
  ...
  getOverwriteInfo(
    ...
    let replace =
      this.editor.getOption(EditorOption.suggest).insertMode === 'replace';
    if (toggleMode) {
      replace = !replace;
    }
