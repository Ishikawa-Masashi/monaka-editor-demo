...
export class SuggestModel implements IDisposable {
  ...
  constructor(
    ...
    this._toDispose.add(
      CompletionProviderRegistry.onDidChange(() => {
        this._updateTriggerCharacters();
  ...
  private _updateQuickSuggest(): void {
    ...
    if (
      isNaN(this._quickSuggestDelay) ||
      (!this._quickSuggestDelay && this._quickSuggestDelay !== 0) ||
      this._quickSuggestDelay < 0
    ) {
  ...
  private _updateTriggerCharacters(): void {
    ...
    >();
    for (const support of CompletionProviderRegistry.all(
    ...
    const checkTriggerCharacter = (text?: string) => {
      ...
      if (supports) {
        ...
        const existing = this._completionModel
          ...
          : undefined;
        this.trigger(
          { auto: true, shy: false, triggerCharacter: lastChar },
          Boolean(this._completionModel),
          supports,
          existing
        );
      }
  ...
  private _updateActiveSuggestSession(): void {
    if (this._state !== State.Idle) {
      if (
        !this._editor.hasModel() ||
        !CompletionProviderRegistry.has(this._editor.getModel())
      ) {
        this.cancel();
      } else {
      ...
      this._triggerQuickSuggest.cancelAndSet(() => {
        ...
        if (quickSuggestions === false) {
          ...
          const tokenType = lineTokens.getStandardTokenType(
            lineTokens.findTokenIndexAtOffset(Math.max(pos.column - 1 - 1, 0))
          );
          const inValidScope =
            (quickSuggestions.other && tokenType === StandardTokenType.Other) ||
  ...
  trigger(
    ...
    existing?: { items: CompletionItem[]; clipboardText: string | undefined }
  ): void {
    if (!this._editor.hasModel()) {
      return;
    }
    ...
    let suggestCtx: CompletionContext = {
      triggerKind: context.triggerKind ?? CompletionTriggerKind.Invoke,
    };
  ...
  private static _createSuggestFilter(editor: ICodeEditor): {
    ...
    return {
      itemKind: result,
      showDeprecated: (suggestOptions as any).showDeprecated,
    };
  }
  ...
  private _onNewContext(ctx: LineContext): void {
    ...
    if (
      ...
      const inactiveProvider = new Set(
        CompletionProviderRegistry.all(this._editor.getModel()!)
      );
      ...
      this.trigger(
        { auto: this._context.auto, shy: false },
        true,
        inactiveProvider,
        { items, clipboardText: this._completionModel.clipboardText }
      );
      return;
    }
    ...
    if (
      ...
      this._completionModel.incomplete.size > 0 &&
      ctx.leadingWord.word.length !== 0
    ) {
      ...
      if (this._completionModel.items.length === 0) {
        if (
          LineContext.shouldAutoTrigger(this._editor) &&
