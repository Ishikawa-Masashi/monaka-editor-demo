...
export async function provideSuggestionItems(
  ...
  context: modes.CompletionContext = {
    triggerKind: modes.CompletionTriggerKind.Invoke,
  },
  ...
  const onCompletionList = (
    ...
    container: modes.CompletionList | null | undefined,
    sw: StopWatch
  ) => {
    if (!container) {
      return;
    ...
    for (let suggestion of container.suggestions) {
      if (!options.kindFilter.has(suggestion.kind)) {
        ...
        if (
          ...
          suggestion?.tags?.includes(modes.CompletionItemTag.Deprecated)
        ) {
          continue;
        }
        ...
        if (
          ...
          suggestion.insertTextRules &
            modes.CompletionItemInsertTextRule.InsertAsSnippet
        ) {
        ...
        result.push(
          new CompletionItem(position, suggestion, container, provider)
        );
      }
    }
    ...
    durations.push({
      providerName: provider._debugDisplayName ?? 'unkown_provider',
  ...
  const snippetCompletions = (async () => {
    if (
      ...
      options.kindFilter.has(modes.CompletionItemKind.Snippet)
    ) {
      return;
    }
    if (
  ...
  for (let providerGroup of modes.CompletionProviderRegistry.orderedGroups(
    model
  )) {
      ...
      const completions = await provideSuggestionItems(
        ...
        undefined,
        {
          ...
          triggerKind: triggerCharacter
            ? modes.CompletionTriggerKind.TriggerCharacter
            : modes.CompletionTriggerKind.Invoke,
        }
      );
...
export function showSimpleSuggestions(
  editor: ICodeEditor,
  suggestions: modes.CompletionItem[]
) {
  setTimeout(() => {
    _provider.onlyOnceSuggestions.push(...suggestions);
    editor
      .getContribution<SuggestController>('editor.contrib.suggestController')
      .triggerSuggest(new Set<modes.CompletionItemProvider>().add(_provider));
  }, 0);
}
