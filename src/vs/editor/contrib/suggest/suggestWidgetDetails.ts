...
export class SuggestDetailsWidget {
  ...
  constructor(
    ...
    this._close = dom.append(
      this._header,
      dom.$('span' + Codicon.close.cssSelector)
    );
  ...
  renderItem(item: CompletionItem, explainMode: boolean): void {
    ...
    if (detail) {
      const cappedDetail =
        detail.length > 100000 ? `${detail.substr(0, 100000)}…` : detail;
...
export class SuggestDetailsOverlay implements IOverlayWidget {
  ...
  _placeAtAnchor(anchorBox: dom.IDomNodePagePosition, size: dom.Dimension) {
    ...
    if (size.width > width) {
      ...
      // position: east, west, SOUTH
      if (
        anchorBox.width > width * 1.3 &&
        bodyBox.height - (anchorBox.top + anchorBox.height) > anchorBox.height
      ) {
        ...
        maxSizeBottom = maxSizeTop.with(
          undefined,
          anchorBox.top - info.verticalPadding
        );
