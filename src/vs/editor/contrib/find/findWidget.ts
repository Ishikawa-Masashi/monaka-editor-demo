...
} from '../../../../vs/editor/contrib/find/findState';
import {
  IContextKey,
  IContextKeyService,
  ...
  implements IOverlayWidget, IVerticalSashLayoutProvider
{
  ...
  private _reveal(): void {
    ...
    if (!this._isVisible) {
      ...
      switch (
        this._codeEditor.getOption(EditorOption.find).autoFindInSelection
      ) {
        case 'always':
          this._toggleSelectionFind.checked = true;
          break;
      ...
      if (
        ...
        if (domNode) {
          ...
          if (this._viewZone && startTop < this._viewZone.heightInPx) {
            ...
            const endLeft =
              editorCoords.left + (endCoords ? endCoords.left : 0);
        ...
        this._contextViewProvider,
        {
          ...
          validation: (value: string): InputBoxMessage | null => {
            ...
            try {
              ...
              return { content: (e as any).message };
            }
          },
          flexibleHeight,
          flexibleWidth,
...
registerThemingParticipant((theme, collector) => {
  ...
  if (widgetShadowColor) {
    collector.addRule(
      `.monaco-editor .find-widget { box-shadow: 0 0 8px 2px ${widgetShadowColor}; }`
    );
  }
  ...
  if (findMatchBorder) {
    collector.addRule(
      `.monaco-editor .currentFindMatch { border: 2px solid ${findMatchBorder}; padding: 1px; box-sizing: border-box; }`
    );
  }
  ...
  if (resizeBorderBackground) {
    collector.addRule(
      `.monaco-editor .find-widget .monaco-sash { background-color: ${resizeBorderBackground}; }`
    );
  } else {
    const border = theme.getColor(editorWidgetBorder);
    if (border) {
      collector.addRule(
        `.monaco-editor .find-widget .monaco-sash { background-color: ${border}; }`
      );
    }
  }
  ...
  if (focusOutline) {
    collector.addRule(
      `.monaco-editor .find-widget .monaco-inputbox.synthetic-focus { outline-color: ${focusOutline}; }`
    );
  }
});
