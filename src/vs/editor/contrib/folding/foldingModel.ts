...
export class FoldingModel {
  ...
  public update(
    newRegions: FoldingRegions,
    blockedLineNumers: number[] = []
  ): void {
    ...
    let isBlocked = (startLineNumber: number, endLineNumber: number) => {
      for (let blockedLineNumber of blockedLineNumers) {
        if (
          startLineNumber < blockedLineNumber &&
          blockedLineNumber <= endLineNumber
        ) {
    ...
    while (collapsedIndex !== -1 && k < newRegions.length) {
      ...
      let decRange = this._textModel.getDecorationRange(
        this._editorDecorationIds[collapsedIndex]
      );
      if (decRange) {
        let collapsedStartLineNumber = decRange.startLineNumber;
        if (
          decRange.startColumn === Math.max(decRange.endColumn - 1, 1) &&
          this._textModel.getLineMaxColumn(collapsedStartLineNumber) ===
            decRange.endColumn
        ) {
  ...
  public getMemento(): CollapseMemento | undefined {
    ...
    for (let i = 0; i < this._regions.length; i++) {
      if (this._regions.isCollapsed(i)) {
        let range = this._textModel.getDecorationRange(
        ...
        if (range) {
          ...
          let endLineNumber =
            range.endLineNumber +
            this._regions.getEndLineNumber(i) -
            this._regions.getStartLineNumber(i);
...
export function toggleCollapseState(
  ...
  for (let lineNumber of lineNumbers) {
    ...
    if (region) {
      ...
      if (levels > 1) {
        let regionsInside = foldingModel.getRegionsInside(
          region,
...
export function setCollapseStateLevelsUp(
  ...
  let toToggle: FoldingRegion[] = [];
  for (let lineNumber of lineNumbers) {
    let regions = foldingModel.getAllRegionsAtLine(
      lineNumber,
...
export function setCollapseStateAtLevel(
  ...
  let filter = (region: FoldingRegion, level: number) =>
    level === foldLevel &&
...
export function setCollapseStateForRest(
  ...
  for (let lineNumber of blockedLineNumbers) {
    filteredRegions.push(
      foldingModel.getAllRegionsAtLine(lineNumber, undefined)[0]
    );
  }
  let filter = (region: FoldingRegion) =>
