...
import {
  ...
  toggleCollapseState,
  setCollapseStateUp,
} from '../../../../vs/editor/contrib/folding/foldingModel';
import {
  FoldingDecorationProvider,
  foldingCollapsedIcon,
  foldingExpandedIcon,
} from './foldingDecorations';
...
import {
  ...
  ID_INIT_PROVIDER,
} from '../../../../vs/editor/contrib/folding/intializingRangeProvider';
  ...
  implements IEditorContribution
{
  ...
  constructor(
    ...
    this.foldingDecorationProvider.autoHideFoldingControls =
      options.get(EditorOption.showFoldingControls) === 'mouseover';
    ...
    this._register(
      this.editor.onDidChangeConfiguration((e: ConfigurationChangedEvent) => {
        ...
        if (e.hasChanged(EditorOption.foldingStrategy)) {
          this._useFoldingProviders =
            this.editor.getOptions().get(EditorOption.foldingStrategy) !==
            'indentation';
  ...
  public saveViewState(): FoldingStateMemento | undefined {
    ...
    if (this.foldingModel) {
      // disposed ?
      let collapsedRegions = this.foldingModel.isInitialized
  ...
  private onModelChanged(): void {
    ...
    this.localToDispose.add(
      this.hiddenRangeModel.onDidChange((hr) => this.onHiddenRangesChanges(hr))
    );
    ...
    this.localToDispose.add(
      FoldingRangeProviderRegistry.onDidChange(() =>
        this.onFoldingStrategyChanged()
      )
    );
    this.localToDispose.add(
  ...
  private getRangeProvider(editorModel: ITextModel): RangeProvider {
    ...
    if (this._useFoldingProviders && this.foldingModel) {
      let foldingProviders = FoldingRangeProviderRegistry.ordered(
      ...
      if (
        ...
        const rangeProvider = (this.rangeProvider =
          new InitializingRangeProvider(
            editorModel,
            this.foldingStateMemento.collapsedRegions,
            () => {
    ...
    if (this.updateScheduler) {
      ...
      this.foldingModelPromise = this.updateScheduler
        .trigger(() => {
          ...
          let foldingRegionPromise = (this.foldingRegionPromise =
            createCancelablePromise((token) =>
              this.getRangeProvider(foldingModel.textModel).compute(token)
            ));
          return foldingRegionPromise.then((foldingRanges) => {
            if (
              ...
              let selectionLineNumbers = selections
                ? selections.map((s) => s.startLineNumber)
                : [];
        ...
        if (foldingModel) {
          ...
          if (region && region.startLineNumber === lineNumber) {
            ...
            if (iconClicked || isCollapsed) {
              ...
              let toToggle = [];
              if (surrounding) {
                let filter = (otherRegion: FoldingRegion) =>
  ...
  constructor() {
    super({
      ...
      kbOpts: {
        ...
        mac: {
          primary:
            KeyMod.CtrlCmd | KeyMod.Alt | KeyCode.US_CLOSE_SQUARE_BRACKET,
        },
...
class FoldAllBlockCommentsAction extends FoldingAction<void> {
  ...
  invoke(
    ...
    if (foldingModel.regions.hasTypes()) {
      ...
      let comments = LanguageConfigurationRegistry.getComments(
...
class FoldAllRegionsAction extends FoldingAction<void> {
  ...
  invoke(
    ...
    if (foldingModel.regions.hasTypes()) {
      ...
      let foldingRules = LanguageConfigurationRegistry.getFoldingRules(
        editorModel.getLanguageIdentifier().id
      );
  ...
  constructor() {
    super({
      ...
      kbOpts: {
        ...
        primary: KeyChord(
          KeyMod.CtrlCmd | KeyCode.KEY_K,
          KeyMod.CtrlCmd | KeyCode.US_MINUS
        ),
        weight: KeybindingWeight.EditorContrib,
      },
    });
...
export const foldBackgroundBackground = registerColor(
...
export const editorFoldForeground = registerColor(
...
registerThemingParticipant((theme, collector) => {
  ...
  if (foldBackground) {
    collector.addRule(
      `.monaco-editor .folded-background { background-color: ${foldBackground}; }`
    );
  }
