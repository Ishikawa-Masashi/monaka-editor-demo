 ...
 * @internal
 */
abstract class BaseEditorOption<K1 extends EditorOption, V>
  implements IEditorOption<K1, V>
{
 ...
 * @internal
 */
abstract class ComputedEditorOption<K1 extends EditorOption, V>
  implements IEditorOption<K1, V>
{
  public readonly id: K1;
...
class SimpleEditorOption<K1 extends EditorOption, V>
  implements IEditorOption<K1, V>
{
  public readonly id: K1;
...
class EditorBooleanOption<K1 extends EditorOption> extends SimpleEditorOption<
  K1,
  boolean
> {
  constructor(
...
class EditorIntOption<K1 extends EditorOption> extends SimpleEditorOption<
  K1,
  number
> {
...
class EditorFloatOption<K1 extends EditorOption> extends SimpleEditorOption<
  K1,
  number
> {
...
class EditorStringOption<K1 extends EditorOption> extends SimpleEditorOption<
  K1,
  string
> {
...
class EditorStringEnumOption<
  ...
  V extends string
> extends SimpleEditorOption<K1, V> {
  ...
  public validate(_input: any): GoToLocationOptions {
    ...
    return {
      ...
      multipleDefinitions:
        ...
        stringSet<GoToLocationValues>(input.multipleDefinitions, 'peek', [
          'peek',
          'gotoAndPeek',
          'goto',
        ]),
      multipleTypeDefinitions:
        ...
        stringSet<GoToLocationValues>(input.multipleTypeDefinitions, 'peek', [
          'peek',
          'gotoAndPeek',
          'goto',
        ]),
      multipleDeclarations:
        ...
        stringSet<GoToLocationValues>(input.multipleDeclarations, 'peek', [
          'peek',
          'gotoAndPeek',
          'goto',
        ]),
      multipleImplementations:
        ...
        stringSet<GoToLocationValues>(input.multipleImplementations, 'peek', [
          'peek',
          'gotoAndPeek',
          'goto',
        ]),
      multipleReferences:
        ...
        stringSet<GoToLocationValues>(input.multipleReferences, 'peek', [
          'peek',
          'gotoAndPeek',
          'goto',
        ]),
...
> {
  ...
  public static computeContainedMinimapLineCount(input: {
    ...
    const extraLinesBeyondLastLine = input.scrollBeyondLastLine
      ? typicalViewportLineCount - 1
      : 0;
    const desiredRatio =
      (input.viewLineCount + extraLinesBeyondLastLine) /
      ...
      if (ratio > 1) {
        ...
        if (minimapSize === 'fit') {
          const effectiveMinimapHeight = Math.ceil(
            (viewLineCount + extraLinesBeyondLastLine) * minimapLineHeight
          );
          if (
            isViewportWrapping &&
            couldUseMemory &&
        ...
        if (minimapSize === 'fill' || fitBecomesFill) {
          ...
          minimapCanvasInnerHeight = Math.ceil(
            Math.max(
              typicalViewportLineCount,
              viewLineCount + extraLinesBeyondLastLine
            ) * minimapLineHeight
          );
          if (isViewportWrapping && fitBecomesFill) {
  ...
  public static computeLayout(
    ...
    if (
      typeof rawLineDecorationsWidth === 'string' &&
      /^\d+(\.\d+)?ch$/.test(rawLineDecorationsWidth)
    ) {
      const multiple = parseFloat(
        rawLineDecorationsWidth.substr(0, rawLineDecorationsWidth.length - 2)
      );
      lineDecorationsWidth = EditorIntOption.clampedInt(
        multiple * typicalHalfwidthCharacterWidth,
        0,
        0,
        1000
      );
...
export type EditorInlineHintsOptions = Readonly<
  Required<IEditorInlineHintsOptions>
>;

class EditorInlineHints extends BaseEditorOption<
  EditorOption.inlineHints,
  EditorInlineHintsOptions
> {
  constructor() {
    const defaults: EditorInlineHintsOptions = {
      enabled: true,
      fontSize: 0,
      fontFamily: EDITOR_FONT_DEFAULTS.fontFamily,
    };
    super(EditorOption.inlineHints, 'inlineHints', defaults, {
...
> {
  ...
  public validate(_input: any): EditorMinimapOptions {
    ...
    return {
      ...
      side: stringSet<'right' | 'left'>(input.side, this.defaultValue.side, [
        'right',
        'left',
      ]),
...
> {
  public override readonly defaultValue: Readonly<
    Required<IQuickSuggestionsOptions>
  >;
...
> {
  constructor() {
    super(EditorOption.scrollbar, 'scrollbar', {
...
> {
  ...
  public validate(_input: any): InternalSuggestOptions {
    ...
    return {
      insertMode: stringSet(input.insertMode, this.defaultValue.insertMode, [
        'insert',
        'replace',
      ]),
      filterGraceful: boolean(
...
> {
  ...
  public validate(input: any): Readonly<Required<ISmartSelectOptions>> {
    ...
    return {
      selectLeadingAndTrailingWhitespace: boolean(
        (input as ISmartSelectOptions).selectLeadingAndTrailingWhitespace,
...
function register<K1 extends EditorOption, V>(
  option: IEditorOption<K1, V>
): IEditorOption<K1, V> {
...
export const EditorOptions = {
  ...
  formatOnType: register(
    new EditorBooleanOption(EditorOption.formatOnType, 'formatOnType', false, {
      description: nls.localize(
        'formatOnType',
  ...
  highlightActiveIndentGuide: register(
    new EditorBooleanOption(
  ...
  inDiffEditor: register(
    new EditorBooleanOption(EditorOption.inDiffEditor, 'inDiffEditor', false)
  ),
  ...
  lineDecorationsWidth: register(
    new SimpleEditorOption(
      EditorOption.lineDecorationsWidth,
      'lineDecorationsWidth',
      10 as number | string
    )
  ...
  renameOnType: register(
    new EditorBooleanOption(EditorOption.renameOnType, 'renameOnType', false, {
      description: nls.localize(
        'renameOnType',
