...
class EditorConfiguration2 {
  ...
  public static validateOptions(
    ...
    for (const editorOption of editorOptionsRegistry) {
      result._write(
        editorOption.id,
        editorOption.validate(options._read(editorOption.id))
      );
    }
    return result;
  }
  ...
  public static checkEquals(
    ...
    for (const editorOption of editorOptionsRegistry) {
      const changed = !EditorConfiguration2._deepEquals(
...
function migrateOptions(options: IEditorOptions): void {
  ...
  if (
    suggest &&
    typeof (<any>suggest).filteredTypes === 'object' &&
    (<any>suggest).filteredTypes
  ) {
...
export abstract class CommonEditorConfiguration
  extends Disposable
  implements IConfiguration
{
  ...
  constructor(isSimpleWidget: boolean, _options: Readonly<IEditorOptions>) {
    ...
    this._validatedOptions = EditorConfiguration2.validateOptions(
      this._readOptions
    );
    ...
    this._register(
      TabFocus.onDidChangeTabFocus((_) => this._recomputeOptions())
    );
  }
  ...
  protected _recomputeOptions(): void {
    ...
    if (!oldOptions) {
      ...
      const changeEvent = EditorConfiguration2.checkEquals(
        oldOptions,
        newOptions
      );
  ...
  public setMaxLineNumber(maxLineNumber: number): void {
    const lineNumbersDigitCount =
      CommonEditorConfiguration._digitCount(maxLineNumber);
