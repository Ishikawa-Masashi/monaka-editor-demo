...
export class EditorWorkerServiceImpl
  extends Disposable
  implements IEditorWorkerService
{
  ...
  constructor(
    ...
    this._register(
      modes.CompletionProviderRegistry.register(
        '*',
  ...
  public computeMoreMinimalEdits(
    resource: URI,
    edits: modes.TextEdit[] | null | undefined
  ): Promise<modes.TextEdit[] | undefined> {
    if (isNonEmptyArray(edits)) {
      ...
      const result = this._workerManager
        .withWorker()
        .then((client) => client.computeMoreMinimalEdits(resource, edits));
      result.finally(() =>
  ...
  public navigateValueSet(
    ...
    range: IRange,
    up: boolean
  ): Promise<modes.IInplaceReplaceSupportResult | null> {
    return this._workerManager
      .withWorker()
...
class WordBasedCompletionItemProvider implements modes.CompletionItemProvider {
  ...
  async provideCompletionItems(
    model: ITextModel,
    position: Position
  ): Promise<modes.CompletionList | undefined> {
    ...
    if (config.wordBasedSuggestionsMode === 'currentDocument') {
      ...
      for (const candidate of this._modelService.getModels()) {
        ...
        if (candidate === model) {
          ...
          config.wordBasedSuggestionsMode === 'allDocuments' ||
          candidate.getLanguageIdentifier().id ===
    ...
    const wordDefRegExp = LanguageConfigurationRegistry.getWordDefinition(
      model.getLanguageIdentifier().id
    );
    ...
    if (!this._worker) {
      try {
        this._worker = this._register(
          new SimpleWorkerClient<EditorSimpleWorker, EditorWorkerHost>(
            this._workerFactory,
  ...
  private _getOrCreateModelManager(
    ...
    if (!this._modelManager) {
      this._modelManager = this._register(
        new EditorModelManager(proxy, this._modelService, this._keepIdleModels)
      );
    }
