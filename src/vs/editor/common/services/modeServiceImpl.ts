...
export class ModeServiceImpl extends Disposable implements IModeService {
  ...
  protected readonly _onLanguagesMaybeChanged = this._register(
    new Emitter<void>({
      leakWarningThreshold: 200 /* https://github.com/microsoft/vscode/issues/119968 */,
    })
  );
  public readonly onLanguagesMaybeChanged: Event<void> =
    this._onLanguagesMaybeChanged.event;
  ...
  public getModeIdByFilepathOrFirstLine(
    resource: URI | null,
    firstLine?: string
  ): string | null {
    const modeIds = this._registry.getModeIdsFromFilepathOrFirstLine(
      resource,
      firstLine
    );
  ...
  public getModeId(
    ...
    const modeIds = this._registry.extractModeIds(
  ...
  private _createModeAndGetLanguageIdentifier(
    ...
    const languageIdentifier =
      this.getLanguageIdentifier(modeId || 'plaintext') ||
  ...
  private _getOrCreateMode(modeId: string): IMode {
    if (!this._instantiatedModes.hasOwnProperty(modeId)) {
      let languageIdentifier =
        this.getLanguageIdentifier(modeId) || NULL_LANGUAGE_IDENTIFIER;
