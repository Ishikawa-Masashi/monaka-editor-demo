...
} from '../../../../vs/editor/common/services/modelService';
...
} from '../../../../vs/editor/common/services/getSemanticTokens';
...
class ModelData implements IDisposable {
  ...
  public setLanguage(languageSelection: ILanguageSelection): void {
    ...
    this._languageSelectionListener = this._languageSelection.onDidChange(() =>
...
export class ModelServiceImpl extends Disposable implements IModelService {
  ...
  private readonly _onModelModeChanged: Emitter<{
    model: ITextModel;
    oldModeId: string;
  }> = this._register(new Emitter<{ model: ITextModel; oldModeId: string }>());
  public readonly onModelModeChanged: Event<{
    model: ITextModel;
    oldModeId: string;
  }> = this._onModelModeChanged.event;
  ...
  constructor(
    ...
    this._semanticStyling = this._register(
      new SemanticStyling(this._themeService, this._logService)
    );
  ...
  private _getEOL(resource: URI | undefined, language: string): string {
    ...
    const eol = this._configurationService.getValue<string>('files.eol', {
  ...
  private _shouldRestoreUndoStack(): boolean {
    const result = this._configurationService.getValue<boolean>(
  ...
  public getCreationOptions(
    ...
    let creationOptions =
      this._modelCreationOptionsByLanguageAndResource[language + resource];
    if (!creationOptions) {
      const editor = this._configurationService.getValue<IRawEditorConfig>(
        'editor',
        {
          overrideIdentifier: language,
          resource,
    ...
    if (resource && this._disposedModels.has(MODEL_ID(resource))) {
      ...
      if (sha1IsEqual || disposedModelData.sharesUndoRedoStack) {
        ...
        this._undoRedoService.setElementsValidFlag(
          ...
          true,
          (element) =>
            isEditStackElement(element) && element.matchesResource(resource)
        );
        if (sha1IsEqual) {
  ...
  public createModel(
    ...
    if (languageSelection) {
      modelData = this._createModelData(
        value,
        languageSelection.languageIdentifier,
        resource,
        isForSimpleWidget
      );
  ...
  public getSemanticTokensProviderStyling(
  ...
  private _onWillDispose(model: ITextModel): void {
    ...
    if (
      ...
      (this._shouldRestoreUndoStack() &&
        schemaShouldMaintainUndoRedoElements(model.uri))
    ) {
    ...
    if (!maintainUndoRedoStack) {
      ...
      this._undoRedoService.setElementsValidFlag(
        ...
        false,
        (element) =>
          isEditStackElement(element) && element.matchesResource(model.uri)
      );
    ...
    delete this._modelCreationOptionsByLanguageAndResource[
      model.getLanguageIdentifier().language + model.uri
    ];
  ...
  constructor(
    ...
    const handleSettingOrThemeChange = () => {
      for (let model of modelService.getModels()) {
        ...
        if (
          isSemanticColoringEnabled(model, themeService, configurationService)
        ) {
          if (!curr) {
            register(model);
          }
...
export class ModelSemanticColoring extends Disposable {
  ...
  constructor(
    ...
    const bindDocumentChangeListeners = () => {
      ...
      this._documentProvidersChangeListeners = [];
      for (const provider of DocumentSemanticTokensProviderRegistry.all(
        model
      )) {
    ...
    this._register(
      DocumentSemanticTokensProviderRegistry.onDidChange(() => {
        bindDocumentChangeListeners();
        this._fetchDocumentSemanticTokens.schedule();
      })
    );
  ...
  private _fetchDocumentSemanticTokensNow(): void {
    ...
    const lastResultId = this._currentDocumentResponse
      ...
      : null;
    const r = getDocumentSemanticTokens(
      this._model,
      lastResultId,
    ...
    request.then(
      ...
      (err) => {
        const isExpectedError =
          err &&
          (errors.isPromiseCanceledError(err) ||
