...
} from '../../../../vs/editor/common/model/mirrorTextModel';
import {
...
export class EditorSimpleWorker implements IRequestHandler, IDisposable {
  ...
  constructor(
    ...
    foreignModuleFactory: IForeignModuleFactory | null
  ) {
    this._host = host;
  ...
  private _modelsAreIdentical(
    original: ICommonModel,
    modified: ICommonModel
  ): boolean {
  ...
  public async computeMoreMinimalEdits(
    ...
    for (let { range, text, eol } of edits) {
      ...
      if (
        Math.max(text.length, original.length) > EditorSimpleWorker._diffLimit
      ) {
  ...
  public loadForeignModule(
    ...
    const foreignHost = types.createProxyObject(
      foreignHostMethods,
      proxyMethodRequest
    );
