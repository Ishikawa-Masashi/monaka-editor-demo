...
import {
  ...
  MultilineTokens2,
  TokensStore2,
} from '../../../../vs/editor/common/model/tokensStore';
...
export function createTextBuffer(
  value: string | model.ITextBufferFactory,
  defaultEOL: model.DefaultEndOfLine
): { textBuffer: model.ITextBuffer; disposable: IDisposable } {
  const factory =
    typeof value === 'string' ? createTextBufferFactory(value) : value;
...
class BracketSearchCanceled {
  ...
  _searchCanceledBrand?: any = undefined;
  private constructor() {}
}
...
export class TextModel extends Disposable implements model.ITextModel {
  ...
  private readonly _onDidChangeDecorations: DidChangeDecorationsEmitter =
    this._register(new DidChangeDecorationsEmitter());
  ...
  public onDidChangeRawContentFast(
    listener: (e: ModelRawContentChangedEvent) => void
  ): IDisposable {
    return this._eventEmitter.fastEvent((e: InternalModelContentChangeEvent) =>
      listener(e.rawContentChangedEvent)
    );
  }
  public onDidChangeContentFast(
  ...
  constructor(
    ...
    this._languageRegistryListener = LanguageConfigurationRegistry.onDidChange(
      (e) => {
  ...
  private _onBeforeEOLChange(): void {
    ...
    const allDecorations = this._decorationsTree.search(
      0,
      false,
      false,
      versionId
  ...
  private _isValidRange(
    ...
    if (validationType === StringOffsetValidationType.SurrogatePairs) {
      ...
      const charCodeBeforeEnd =
        endColumn > 1 && endColumn <= this._buffer.getLineLength(endLineNumber)
  ...
  private _validateEditOperations(
    rawOperations: model.IIdentifiedSingleEditOperation[]
  ): model.ValidAnnotatedEditOperation[] {
  ...
  _applyUndo(
    ...
    const edits = changes.map<model.IIdentifiedSingleEditOperation>(
      (change) => {
  ...
  private _applyUndoRedoEdits(
    edits: model.IIdentifiedSingleEditOperation[],
    eol: model.EndOfLineSequence,
    isUndoing: boolean,
    isRedoing: boolean,
    resultingAlternativeVersionId: number,
  ...
  public applyEdits(
    operations: model.IIdentifiedSingleEditOperation[],
    ...
    if (contentChanges.length !== 0) {
      ...
      for (let i = 0, len = contentChanges.length; i < len; i++) {
        ...
        this._tokens2.acceptEdit(
          ...
          firstLineLength,
          lastLineLength,
          change.text.length > 0 ? change.text.charCodeAt(0) : CharCode.Null
        );
        ...
        for (let j = editingLinesCnt; j >= 0; j--) {
          ...
          const currentEditLineNumber =
            newLineCount - lineCount - changeLineCountDelta + editLineNumber;
        ...
        if (editingLinesCnt < insertingLinesCnt) {
          ...
          for (let i = 0; i < cnt; i++) {
            ...
            newLines[lineNumber - fromLineNumber] =
  ...
  public getOverviewRulerDecorations(
    ...
    const versionId = this.getVersionId();
    const result = this._decorationsTree.search(
      ownerId,
      filterOutValidation,
      true,
      versionId
  ...
  private _getDecorationsInRange(
    ...
    const versionId = this.getVersionId();
    const result = this._decorationsTree.intervalSearch(
      startOffset,
      endOffset,
      filterOwnerId,
      filterOutValidation,
      ...
      for (let j = 0, lenJ = element.tokens.length; j < lenJ; j++) {
        ...
        if (hasChange) {
          this._tokens.setTokens(
            ...
            lineNumber - 1,
            this._buffer.getLineLength(lineNumber),
            element.tokens[j],
            false
          );
  ...
  public getWordAtPosition(_position: IPosition): model.IWordAtPosition | null {
    ...
    const [rbStartOffset, rbEndOffset] = TextModel._findLanguageBoundaries(
      lineTokens,
      tokenIndex
    );
    ...
    if (tokenIndex > 0 && rbStartOffset === position.column - 1) {
      ...
      const [lbStartOffset, lbEndOffset] = TextModel._findLanguageBoundaries(
        lineTokens,
        tokenIndex - 1
      );
  ...
  public findMatchingBracketUp(
    ...
    let languageId = lineTokens.getLanguageId(
      lineTokens.findTokenIndexAtOffset(position.column - 1)
    );
    let bracketsSupport =
      LanguageConfigurationRegistry.getBracketsSupport(languageId);
    ...
    return stripBracketSearchCanceled(
      this._findMatchingBracketUp(data, position, null)
    );
  }
  ...
  private _matchBracket(position: Position): [Range, Range] | null {
    ...
    const currentModeBrackets =
      LanguageConfigurationRegistry.getBracketsSupport(
        lineTokens.getLanguageId(tokenIndex)
      );
  ...
  public findEnclosingBrackets(
    ...
    const resetCounts = (
      ...
      if (!savedCounts.has(languageId)) {
        let tmp = [];
        for (
          let i = 0, len = modeBrackets ? modeBrackets.brackets.length : 0;
          i < len;
          i++
        ) {
          tmp[i] = 0;
  ...
  private _computeIndentLevel(lineIndex: number): number {
    return TextModel.computeIndentLevel(
      this._buffer.getLineContent(lineIndex + 1),
      this._options.tabSize
    );
  }
  ...
  public getActiveIndentGuide(
    ...
    const foldingRules = LanguageConfigurationRegistry.getFoldingRules(
      this._languageIdentifier.id
    );
      ...
      if (goUp) {
        ...
        if (currentIndent >= 0) {
          ...
          upLineIndentLevel = Math.ceil(
            currentIndent / this._options.indentSize
          );
        } else {
  ...
  private _getIndentLevelForWhitespaceLine(
    ...
    if (aboveContentLineIndent === -1 || belowContentLineIndent === -1) {
      ...
      if (offSide) {
        ...
        return (
          1 + Math.floor(belowContentLineIndent / this._options.indentSize)
        );
      }
    }
  }
  ...
  normalizePosition(
    position: Position,
    affinity: model.PositionNormalizationAffinity
  ): Position {
    return position;
  }
