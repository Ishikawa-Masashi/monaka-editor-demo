...
import {
  ...
  righttest,
  updateTreeMetadata,
} from '../../../../../vs/editor/common/model/pieceTreeTextBuffer/rbTreeBase';
import {
  SearchData,
  Searcher,
  createFindMatch,
...
export class PieceTreeBase {
  ...
  public findMatchesInNode(
    ...
    let startOffsetInBuffer = this.offsetInBuffer(
      node.piece.bufferIndex,
      node.piece.start
    );
    ...
    do {
      ...
      if (m) {
        ...
        let lineFeedCnt = this.getLineFeedCnt(
          node.piece.bufferIndex,
          startCursor,
          ret
        );
  ...
  public findMatchesLineByLine(
    ...
    while (currentNode !== endPosition.node) {
      let lineBreakCnt = this.getLineFeedCnt(
  ...
  public getLineRawContent(lineNumber: number, endOffset: number = 0): string {
    ...
    if (cache) {
      x = cache.node;
      let prevAccumulatedValue = this.getAccumulatedValue(
        x,
      ...
      if (cache.nodeStartLineNumber + x.piece.lineFeedCnt === lineNumber) {
        ...
        let accumulatedValue = this.getAccumulatedValue(
          x,
