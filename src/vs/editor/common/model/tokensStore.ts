...
export class SparseEncodedTokens {
  ...
  public toString(startLineNumber: number): string {
    ...
    for (let i = 0; i < this._tokenCount; i++) {
      pieces.push(
        `(${this._getDeltaLine(i) + startLineNumber},${this._getStartCharacter(
          i
        )}-${this._getEndCharacter(i)})`
      );
    }
    return `[${pieces.join(',')}]`;
  }
  ...
  public split(
    ...
    endDeltaLine: number,
    endChar: number
  ): [SparseEncodedTokens, SparseEncodedTokens, number] {
    const tokens = this._tokens;
    const tokenCount = this._tokenCount;
    let aTokens: number[] = [];
    let bTokens: number[] = [];
    ...
    return [
      new SparseEncodedTokens(new Uint32Array(aTokens)),
      new SparseEncodedTokens(new Uint32Array(bTokens)),
      destFirstDeltaLine,
    ];
  }
      ...
      if (
        ...
        if (
          ...
          if (tokenDeltaLine === startDeltaLine) {
            ...
            tokenStartCharacter = startCharacter;
            tokenEndCharacter =
              tokenStartCharacter + (tokenEndCharacter - endCharacter);
          } else {
...
export class MultilineTokens2 {
  ...
  public getRange(): Range | null {
    ...
    return new Range(
      this.startLineNumber + deltaRange.startLineNumber,
      deltaRange.startColumn,
      this.startLineNumber + deltaRange.endLineNumber,
  ...
  public removeTokens(range: Range): void {
    ...
    this.startLineNumber += this.tokens.removeTokens(
      startLineIndex,
      range.startColumn - 1,
      endLineIndex,
      range.endColumn - 1
  ...
  public split(range: Range): [MultilineTokens2, MultilineTokens2] {
    ...
    const [a, b, bDeltaLine] = this.tokens.split(
      startLineIndex,
      range.startColumn - 1,
      endLineIndex,
      range.endColumn - 1
    ...
    return [
      new MultilineTokens2(this.startLineNumber, a),
      new MultilineTokens2(this.startLineNumber + bDeltaLine, b),
    ];
  }
...
export class TokensStore {
  ...
  public setTokens(
    ...
    const tokens = TokensStore._massageTokens(
      topLevelLanguageId,
      lineTextLength,
      _tokens
    );
  ...
  private _acceptDeleteRange(range: IRange): void {
    ...
    if (range.startLineNumber === range.endLineNumber) {
      ...
      this._lineTokens[firstLineIndex] = TokensStore._delete(
    ...
    this._lineTokens[firstLineIndex] = TokensStore._deleteEnding(
      this._lineTokens[firstLineIndex],
      range.startColumn - 1
    );
    ...
    if (lastLineIndex < this._len) {
      lastLineTokens = TokensStore._deleteBeginning(
    ...
    this._lineTokens[firstLineIndex] = TokensStore._append(
      this._lineTokens[firstLineIndex],
      lastLineTokens
    );
  ...
  private _acceptInsertText(
    ...
    if (eolCount === 0) {
      ...
      this._lineTokens[lineIndex] = TokensStore._insert(
    ...
    this._lineTokens[lineIndex] = TokensStore._deleteEnding(
      this._lineTokens[lineIndex],
      position.column - 1
    );
    this._lineTokens[lineIndex] = TokensStore._insert(
  ...
  public static _deleteBeginning(
