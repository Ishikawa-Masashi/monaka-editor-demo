...
export class TokenizationStateStore {
  ...
  private _insertLines(insertIndex: number, insertCount: number): void {
    ...
    this._beginState = arrays.arrayInsert(
      this._beginState,
      insertIndex,
      beginState
    );
...
export class TextModelTokenization extends Disposable {
  ...
  constructor(textModel: TextModel) {
    ...
    this._register(
      this._textModel.onDidChangeRawContentFast((e) => {
    ...
    this._register(
      this._textModel.onDidChangeContentFast((e) => {
    ...
    this._register(
      this._textModel.onDidChangeAttached(() => {
        this._beginBackgroundTokenization();
      })
    );
  ...
  private _revalidateTokensNow(
    toLineNumber: number = this._textModel.getLineCount()
  ): void {
  ...
  public isCheapToTokenize(lineNumber: number): boolean {
    ...
    const firstInvalidLineNumber =
      this._tokenizationStateStore.invalidLineStartIndex + 1;
  ...
  private _hasLinesToTokenize(): boolean {
    ...
    return (
      this._tokenizationStateStore.invalidLineStartIndex <
  ...
  private _updateTokensUntilLine(
    ...
    for (
      let lineIndex = this._tokenizationStateStore.invalidLineStartIndex;
      lineIndex <= endLineIndex;
      lineIndex++
    ) {
      const text = this._textModel.getLineContent(lineIndex + 1);
      const lineStartState =
        this._tokenizationStateStore.getBeginState(lineIndex);
  ...
  private _tokenizeViewport(
    ...
    let nonWhitespaceColumn =
      this._textModel.getLineFirstNonWhitespaceColumn(startLineNumber);
    let fakeLines: string[] = [];
    let initialState: IState | null = null;
    for (let i = startLineNumber - 1; nonWhitespaceColumn > 0 && i >= 1; i--) {
      let newNonWhitespaceIndex =
...
function initializeTokenization(
  textModel: TextModel
): [ITokenizationSupport | null, IState | null] {
