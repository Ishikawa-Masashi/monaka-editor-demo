...
export function getNodeIsInOverviewRuler(node: IntervalNode): boolean {
  return (
    (node.metadata & Constants.IsInOverviewRulerMask) >>>
      Constants.IsInOverviewRulerOffset ===
    1
  );
}
function setNodeIsInOverviewRuler(node: IntervalNode, value: boolean): void {
  node.metadata =
    (node.metadata & Constants.IsInOverviewRulerMaskInverse) |
    ((value ? 1 : 0) << Constants.IsInOverviewRulerOffset);
}
...
export class IntervalNode implements IModelDecoration {
  ...
  public setOptions(options: ModelDecorationOptions) {
    ...
    setNodeIsInOverviewRuler(
      this,
      this.options.overviewRuler && this.options.overviewRuler.color
        ? true
        : false
    );
