...
class EditorScrollable extends Disposable {
  ...
  constructor(
    ...
    this._scrollable = this._register(
      new Scrollable(smoothScrollDuration, scheduleAtNextAnimationFrame)
    );
...
export class ViewLayout extends Disposable implements IViewLayout {
  ...
  private _computeContentWidth(maxLineWidth: number): number {
    ...
    if (wrappingInfo.isViewportWrapping) {
      ...
      if (
        maxLineWidth >
        layoutInfo.contentWidth + fontInfo.typicalHalfwidthCharacterWidth
      ) {
