...
import {
  IViewLineTokens,
  LineTokens,
} from '../../../../vs/editor/common/core/lineTokens';
...
export function tokenizeToString(
  text: string,
  tokenizationSupport: IReducedTokenizationSupport = fallback
): string {
...
export function tokenizeLineToHTML(
  ...
  for (
    ...
    result += `<span style="${viewLineTokens.getInlineStyle(
      tokenIndex,
      colorMap
    )}">${partContent}</span>`;
...
function _tokenizeToString(
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    for (let j = 0, lenJ = viewLineTokens.getCount(); j < lenJ; j++) {
      ...
      result += `<span class="${type}">${strings.escape(
        line.substring(startOffset, endIndex)
      )}</span>`;
