...
import {
  IndentConsts,
  IndentRulesSupport,
} from '../../../../vs/editor/common/modes/supports/indentRules';
...
export class RichEditSupport {
  ...
  constructor(
    ...
    this._onEnterSupport =
      this._conf.brackets ||
      this._conf.indentationRules ||
      this._conf.onEnterRules
        ? new OnEnterSupport(this._conf)
        : null;
...
class LanguageConfigurationEntry {
  ...
  public static cmp(
    a: LanguageConfigurationEntry,
    b: LanguageConfigurationEntry
  ) {
...
class LanguageConfigurationEntries {
  ...
  public register(
    ...
    const entry = new LanguageConfigurationEntry(
      configuration,
      priority,
      ++this._order
    );
  ...
  private _resolve(): LanguageConfiguration | null {
    ...
    for (const entry of this._entries) {
      ...
      result.indentationRules =
        conf.indentationRules || result.indentationRules;
      result.onEnterRules = conf.onEnterRules || result.onEnterRules;
      result.autoClosingPairs =
        conf.autoClosingPairs || result.autoClosingPairs;
      result.surroundingPairs =
        conf.surroundingPairs || result.surroundingPairs;
      ...
      result.__electricCharacterSupport =
        conf.__electricCharacterSupport || result.__electricCharacterSupport;
    }
    return result;
  }
}
...
export class LanguageConfigurationRegistryImpl {
  private readonly _entries2 = new Map<
    LanguageId,
    LanguageConfigurationEntries
  >();
  ...
  public register(
    ...
    this._onDidChange.fire(
      new LanguageConfigurationChangeEvent(languageIdentifier)
    );
  ...
  ): BracketElectricCharacterSupport | null {
  ...
  public getElectricCharacters(languageId: LanguageId): string[] {
    let electricCharacterSupport =
      this._getElectricCharacterSupport(languageId);
  ...
  public getAutoClosingPairs(languageId: LanguageId): AutoClosingPairs {
    ...
    return new AutoClosingPairs(
      characterPairSupport ? characterPairSupport.getAutoClosingPairs() : []
    );
  }
   ...
   */
  public getInheritIndentForLine(
    autoIndent: EditorAutoIndentStrategy,
    model: IVirtualModel,
    lineNumber: number,
    ...
    const indentRulesSupport = this.getIndentRulesSupport(
    ...
    const precedingUnIgnoredLine = this.getPrecedingValidLine(
      model,
      lineNumber,
      indentRulesSupport
    );
    ...
    if (
      ...
      indentRulesSupport.shouldDecrease(precedingUnIgnoredLineContent)
    ) {
      return {
        indentation: strings.getLeadingWhitespace(
          precedingUnIgnoredLineContent
      ...
      if (
        ...
        for (let i = previousLine - 1; i > 0; i--) {
          if (
            indentRulesSupport.shouldIndentNextLine(model.getLineContent(i))
          ) {
            continue;
          }
          stopLine = i;
  ...
  public getGoodIndentForLine(
    ...
    const indent = this.getInheritIndentForLine(
      autoIndent,
      virtualModel,
      lineNumber
    );
  ...
  public getIndentForEnter(
    ...
    const afterEnterAction = this.getInheritIndentForLine(
      autoIndent,
      virtualModel,
    ...
    let afterEnterIndent = embeddedLanguage
      ? currentLineIndent
  ...
  public getIndentActionForType(
    ...
    const scopedLineTokens = this.getScopedLineTokens(
      model,
  ...
  public getEnterAction(
    ...
    const richEditSupport = this._getRichEditSupport(
      scopedLineTokens.languageId
    );
    if (!richEditSupport) {
      return null;
    ...
    const scopedLineText = scopedLineTokens.getLineContent();
    const beforeEnterText = scopedLineText.substr(
      0,
      range.startColumn - 1 - scopedLineTokens.firstCharOffset
    );
    ...
    if (range.startLineNumber > 1 && scopedLineTokens.firstCharOffset === 0) {
      ...
      const oneLineAboveScopedLineTokens = this.getScopedLineTokens(
        model,
        range.startLineNumber - 1
      );
      if (
        oneLineAboveScopedLineTokens.languageId === scopedLineTokens.languageId
      ) {
