...
export const editorBracketMatchBorder = registerColor(
  ...
  { dark: '#888', light: '#B9B9B9', hc: contrastBorder },
...
export const ghostTextForeground = registerColor(
  ...
  { dark: Color.fromHex('#ffffff56'), light: Color.fromHex('#0007'), hc: null },
  nls.localize(
...
registerThemingParticipant((theme, collector) => {
  ...
  if (background) {
    collector.addRule(
      `.monaco-editor, .monaco-editor-background, .monaco-editor .inputarea.ime-input { background-color: ${background}; }`
    );
  }
  ...
  if (symbolHighlight) {
    collector.addRule(
      `.monaco-editor .symbolHighlight { background-color: ${symbolHighlight}; }`
    );
  }
  ...
  if (invisibles) {
    collector.addRule(
      `.monaco-editor .mtkw { color: ${invisibles} !important; }`
    );
    collector.addRule(
      `.monaco-editor .mtkz { color: ${invisibles} !important; }`
    );
  }
});
