...
export class DeleteOperations {
  ...
  public static deleteLeft(
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      let deleteRange = DeleteOperations.getDeleteRange(
        selections[i],
        model,
        config
      );
  ...
  private static getDeleteRange(
    ...
    if (config.useTabStops && position.column > 1) {
      ...
      if (position.column <= lastIndentationColumn) {
        const fromVisibleColumn = CursorColumns.visibleColumnFromColumn2(
          config,
          model,
          position
        );
        ...
        const toColumn = CursorColumns.columnFromVisibleColumn2(
          config,
          model,
          position.lineNumber,
          toVisibleColumn
