...
export class CursorConfiguration {
  ...
  constructor(
    ...
    this.shouldAutoCloseBefore = {
      quote: CursorConfiguration._getShouldAutoClose(
        languageIdentifier,
        this.autoClosingQuotes
      ),
      bracket: CursorConfiguration._getShouldAutoClose(
        languageIdentifier,
        this.autoClosingBrackets
      ),
    };

    this.autoClosingPairs = LanguageConfigurationRegistry.getAutoClosingPairs(
      languageIdentifier.id
    );

    let surroundingPairs =
      CursorConfiguration._getSurroundingPairs(languageIdentifier);
    if (surroundingPairs) {
  ...
  private static _getShouldAutoClose(
    ...
    switch (autoCloseConfig) {
      ...
      case 'languageDefined':
        return CursorConfiguration._getLanguageDefinedShouldAutoClose(
          languageIdentifier
        );
      case 'always':
  ...
  private static _getLanguageDefinedShouldAutoClose(
    ...
    try {
      const autoCloseBeforeSet =
        LanguageConfigurationRegistry.getAutoCloseBeforeSet(
          languageIdentifier.id
        );
...
export class CursorColumns {
  public static visibleColumnFromColumn(
    ...
    const endOffset =
      column - 1 < lineContentLength ? column - 1 : lineContentLength;
    ...
    while (i < endOffset) {
      ...
      if (codePoint === CharCode.Tab) {
        ...
        while (i < endOffset) {
          const nextCodePoint = strings.getNextCodePoint(
            lineContent,
            endOffset,
            i
          );
          const nextGraphemeBreakType =
            strings.getGraphemeBreakType(nextCodePoint);
          if (
            strings.breakBetweenGraphemeBreakType(
              graphemeBreakType,
          ...
          i +=
            nextCodePoint >= Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN
              ? 2
              : 1;
   ...
   */
  public static visibleColumnsByColumns(
    lineContent: string,
    tabSize: number
  ): number[] {
  ...
  public static columnFromVisibleColumn(
    ...
    while (i < lineLength) {
      ...
      if (codePoint === CharCode.Tab) {
        afterVisibleColumn = CursorColumns.nextRenderTabStop(
          beforeVisibleColumn,
          tabSize
        );
      } else {
