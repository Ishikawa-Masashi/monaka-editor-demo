...
export class TypeOperations {
  ...
  private static _goodIndentForLine(
    ...
    const expectedIndentAction =
      LanguageConfigurationRegistry.getInheritIndentForLine(
        config.autoIndent,
        model,
        lineNumber,
        false
    ...
    if (expectedIndentAction) {
      ...
      const expectedEnterAction = LanguageConfigurationRegistry.getEnterAction(
        config.autoIndent,
        model,
  ...
  public static tab(
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      if (selection.isEmpty()) {
        ...
        if (
          /^\s*$/.test(lineText) &&
          model.isCheapToTokenize(selection.startLineNumber)
        ) {
  ...
  private static _enter(
    ...
    if (
      !model.isCheapToTokenize(range.getStartPosition().lineNumber) ||
      config.autoIndent === EditorAutoIndentStrategy.Keep
    ) {
      let lineText = model.getLineContent(range.startLineNumber);
      let indentation = strings
    ...
    const r = LanguageConfigurationRegistry.getEnterAction(
      config.autoIndent,
      model,
      range
    );
    ...
    if (config.autoIndent >= EditorAutoIndentStrategy.Full) {
      const ir = LanguageConfigurationRegistry.getIndentForEnter(
        config.autoIndent,
        model,
        range,
        {
      ...
      if (ir) {
        let oldEndViewColumn = CursorColumns.visibleColumnFromColumn2(
          config,
          model,
        ...
        if (keepPosition) {
          ...
          if (oldEndColumn <= firstNonWhitespace + 1) {
            ...
            offset = Math.min(
              oldEndViewColumn +
                1 -
                config.normalizeIndentation(ir.afterEnter).length -
                1,
              0
            );
          }
  ...
  private static _runAutoIndentType(
    ...
    const currentIndentation =
      LanguageConfigurationRegistry.getIndentationAtPosition(
        model,
        range.startLineNumber,
        range.startColumn
      );
    const actualIndentation =
      LanguageConfigurationRegistry.getIndentActionForType(
        config.autoIndent,
        model,
        range,
        ch,
        ...
        return TypeOperations._typeCommand(
          ...
          config.normalizeIndentation(actualIndentation) +
            model
              ...
              .substring(firstNonWhitespace - 1, range.startColumn - 1) +
            ch,
          false
        );
      }
  ...
  private static _isBeforeClosingBrace(
    ...
    const potentialClosingBraces =
      config.autoClosingPairs.autoClosingPairsCloseByStart.get(nextChar) || [];
  ...
  private static _findAutoClosingPairOpen(
    ...
    for (const autoClosingPairCandidate of autoClosingPairCandidates) {
      if (
        ...
        autoClosingPairCandidate.open.length > autoClosingPair.open.length
      ) {
        ...
        for (const position of positions) {
          const relevantText = model.getValueInRange(
            new Range(
              position.lineNumber,
              position.column - autoClosingPairCandidate.open.length + 1,
              position.lineNumber,
              position.column
            )
          );
  ...
  private static _findSubAutoClosingPairClose(
    ...
    const lastChar = autoClosingPair.close.charAt(
      autoClosingPair.close.length - 1
    );
    // get candidates with the same last character as close
    const subPairCandidates =
    ...
    for (const x of subPairCandidates) {
      if (
        x.open !== autoClosingPair.open &&
        autoClosingPair.open.includes(x.open) &&
        autoClosingPair.close.endsWith(x.close)
      ) {
  ...
  private static _getAutoClosingPairClose(
    ...
    const chIsQuote = isQuote(ch);
    const autoCloseConfig = chIsQuote
    ...
    const autoClosingPair = this._findAutoClosingPairOpen(
      config,
      model,
    ...
    const subAutoClosingPairClose = this._findSubAutoClosingPairClose(
      config,
      autoClosingPair
    );
    let isSubAutoClosingPairPresent = true;

    const shouldAutoCloseBefore = chIsQuote
      ? config.shouldAutoCloseBefore.quote
      : config.shouldAutoCloseBefore.bracket;
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      if (
        ...
        if (
          insertOpenCharacter &&
          position.column > 1 &&
          wordSeparators.get(lineText.charCodeAt(position.column - 2)) ===
        ...
        if (
          !insertOpenCharacter &&
          position.column > 2 &&
          wordSeparators.get(lineText.charCodeAt(position.column - 3)) ===
            WordCharacterClass.Regular
        ) {
          return null;
        }
      ...
      try {
        shouldAutoClosePair = LanguageConfigurationRegistry.shouldAutoClosePair(
          autoClosingPair,
          lineTokens,
          insertOpenCharacter ? position.column : position.column - 1
        );
      } catch (e) {
        onUnexpectedError(e);
      }
    ...
    if (isSubAutoClosingPairPresent) {
      return autoClosingPair.close.substring(
        0,
        autoClosingPair.close.length - subAutoClosingPairClose.length
      );
    } else {
  ...
  private static _isTypeInterceptorElectricChar(
    ...
    if (
      selections.length === 1 &&
      model.isCheapToTokenize(selections[0].getEndPosition().lineNumber)
    ) {
      return true;
    }
    return false;
  ...
  public static compositionEndWithInterceptors(
    ...
    config: CursorConfiguration,
    model: ITextModel,
    selectionsWhenCompositionStarted: Selection[] | null,
    selections: Selection[],
    autoClosedCharacters: Range[]
  ): EditOperationResult | null {
    if (
      !selectionsWhenCompositionStarted ||
      Selection.selectionsArrEqual(selectionsWhenCompositionStarted, selections)
    ) {
