...
export interface ISplitLine {
  ...
  normalizePosition(
    ...
    affinity: PositionNormalizationAffinity
  ): Position;
}
...
export interface IViewModelLinesCollection extends IDisposable {
  ...
  onModelLinesInserted(
    ...
    fromLineNumber: number,
    toLineNumber: number,
    lineBreaks: (LineBreakData | null)[]
  ): viewEvents.ViewLinesInsertedEvent | null;
  onModelLineChanged(
    versionId: number,
    lineNumber: number,
    lineBreakData: LineBreakData | null
  ): [
    boolean,
...
class LineNumberMapper {
  ...
  public getIndexOf(accumulatedValue: number): PrefixSumIndexOfResult {
    ...
    const viewLinesAbove =
      modelLineIndex > 0 ? this._modelToView[modelLineIndex - 1] : 0;
    return new PrefixSumIndexOfResult(
      modelLineIndex,
      accumulatedValue - viewLinesAbove
    );
  }
}
...
export class SplitLinesCollection implements IViewModelLinesCollection {
  ...
  private _constructLines(
    ...
    previousLineBreaks: (LineBreakData | null)[] | null
  ): void {
    this.lines = [];
  ...
  public onModelLinesInserted(
    ...
    const isInHiddenArea =
      fromLineNumber > 2 && !this.lines[fromLineNumber - 2].isVisible();
  ...
  public getActiveIndentGuide(
    ...
    const result = this.model.getActiveIndentGuide(
      modelPosition.lineNumber,
  ...
  public getViewLinesIndentGuides(
    ...
    for (
      ...
      if (line.isVisible()) {
        let viewLineStartIndex = line.getViewLineNumberOfModelPosition(
          0,
          modelLineIndex === modelStartLineIndex ? modelStart.column : 1
        );
        let viewLineEndIndex = line.getViewLineNumberOfModelPosition(
          0,
        ...
        if (reqStart !== null) {
          result = result.concat(
            this.model.getLinesIndentGuides(reqStart.lineNumber, modelLineIndex)
          );
          reqStart = null;
        }
      }
    ...
    if (reqStart !== null) {
      result = result.concat(
        this.model.getLinesIndentGuides(
          reqStart.lineNumber,
          modelEnd.lineNumber
        )
      );
  ...
  public getViewLineLength(viewLineNumber: number): number {
    ...
    return this.lines[lineIndex].getViewLineLength(
      this.model,
      lineIndex + 1,
      remainder
    );
  ...
  public convertModelRangeToViewRange(modelRange: Range): Range {
    let start = this.convertModelPositionToViewPosition(
      modelRange.startLineNumber,
      modelRange.startColumn
    );
    let end = this.convertModelPositionToViewPosition(
      modelRange.endLineNumber,
      modelRange.endColumn
    );
    if (
      modelRange.startLineNumber === modelRange.endLineNumber &&
      ...
      if (end.column === this.getViewLineMinColumn(end.lineNumber)) {
        ...
        return new Range(
          ...
          this.getViewLineMaxColumn(end.lineNumber - 1)
        );
      }
    }
    return new Range(
    ...
    if (this.lines[lineIndex].isVisible()) {
      ...
      const deltaLineNumber =
        ...
        (lineIndex === 0
          ? 0
          : this.prefixSumComputer.getAccumulatedValue(lineIndex - 1));
      return this.lines[lineIndex].getViewLineNumberOfModelPosition(
        deltaLineNumber,
        inputColumn
      );
    }
  ...
  public getAllOverviewRulerDecorations(
    ...
    for (const decoration of decorations) {
      const opts = <ModelDecorationOverviewRulerOptions>(
      ...
      const viewStartLineNumber = this._getViewLineNumberForModelPosition(
        decoration.range.startLineNumber,
        decoration.range.startColumn
      );
      const viewEndLineNumber = this._getViewLineNumberForModelPosition(
  ...
  public getDecorationsInRange(
    ...
    let finalResult: IModelDecoration[] = [],
      finalResultLen = 0;
  ...
  normalizePosition(
    ...
    return this.lines[lineIndex].normalizePosition(
      this.model,
      lineIndex + 1,
      remainder,
      position,
...
export class SplitLine implements ISplitLine {
  ...
  private getInputStartOffsetOfOutputLineIndex(
    ...
    return LineBreakData.getInputOffsetOfOutputPosition(
      this._lineBreakData.breakOffsets,
      outputLineIndex,
      0
    );
  }
  ...
  public getViewLineContent(
    ...
    let startOffset =
      this.getInputStartOffsetOfOutputLineIndex(outputLineIndex);
  ...
  public getViewLineData(
    ...
    if (outputLineIndex > 0) {
      lineContent =
        spaces(this._lineBreakData.wrappedTextIndentLength) + lineContent;
    }

    let minColumn =
      outputLineIndex > 0 ? this._lineBreakData.wrappedTextIndentLength + 1 : 1;
    let maxColumn = lineContent.length + 1;

    let continuesWithWrappedLine =
    ...
    const startVisibleColumn =
      outputLineIndex === 0
        ? 0
        : this._lineBreakData.breakOffsetsVisibleColumn[outputLineIndex - 1];
  ...
  public getModelColumnOfViewPosition(
    ...
    return (
      LineBreakData.getInputOffsetOfOutputPosition(
        this._lineBreakData.breakOffsets,
        outputLineIndex,
        adjustedColumn
      ) + 1
  ...
  public getViewPositionOfModelPosition(
    ...
    let r = LineBreakData.getOutputPositionOfInputOffset(
      this._lineBreakData.breakOffsets,
      inputColumn - 1
    );
  ...
  public getViewLineNumberOfModelPosition(
    ...
    const r = LineBreakData.getOutputPositionOfInputOffset(
      this._lineBreakData.breakOffsets,
      inputColumn - 1
    );
  ...
  public normalizePosition(
    ...
    if (affinity === PositionNormalizationAffinity.Left) {
      if (
        outputLineIndex > 0 &&
        outputPosition.column === this._getViewLineMinColumn(outputLineIndex)
      ) {
        return new Position(
          outputPosition.lineNumber - 1,
          this.getViewLineMaxColumn(model, modelLineNumber, outputLineIndex - 1)
        );
      }
      ...
      if (
        ...
        outputPosition.column ===
          this.getViewLineMaxColumn(model, modelLineNumber, outputLineIndex)
      ) {
        return new Position(
          outputPosition.lineNumber + 1,
          this._getViewLineMinColumn(outputLineIndex + 1)
        );
      }
    }
    return outputPosition;
...
export class IdentityLinesCollection implements IViewModelLinesCollection {
  ...
  public createLineBreaksComputer(): ILineBreaksComputer {
    ...
    return {
      addRequest: (
        lineText: string,
        previousLineBreakData: LineBreakData | null
      ) => {
        result.push(null);
      },
      finalize: () => {
  ...
  public onModelLineChanged(
    ...
    return [
      false,
      new viewEvents.ViewLinesChangedEvent(lineNumber, lineNumber),
      null,
      null,
    ];
  }
