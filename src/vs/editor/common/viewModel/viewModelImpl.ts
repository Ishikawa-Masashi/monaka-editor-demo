...
} from '../../../../vs/editor/common/viewModel/viewModelEventDispatcher';
...
export class ViewModel extends Disposable implements IViewModel {
  ...
  constructor(
    ...
    this._tokenizeViewportSoon = this._register(
      new RunOnceScheduler(() => this.tokenizeViewport(), 50)
    );
    ...
    this._cursor = this._register(
      new Cursor(model, this, this.coordinatesConverter, this.cursorConfig)
    );
  ...
  public tokenizeViewport(): void {
    ...
    const startPosition =
      this.coordinatesConverter.convertViewPositionToModelPosition(
        new Position(linesViewportData.startLineNumber, 1)
      );
    const endPosition =
      this.coordinatesConverter.convertViewPositionToModelPosition(
        new Position(linesViewportData.endLineNumber, 1)
      );
    this.model.tokenizeViewport(
  ...
  public onDidColorThemeChange(): void {
    this._eventDispatcher.emitSingleViewEvent(
      new viewEvents.ViewThemeChangedEvent()
    );
  }
  ...
  private _onConfigurationChanged(
    ...
    if (this._viewportStartLine !== -1) {
      let previousViewportStartViewPosition = new Position(
  ...
  private _registerModelEvents(): void {
    this._register(
      this.model.onDidChangeRawContentFast((e) => {
        try {
          const eventsCollector = this._eventDispatcher.beginEmitViewEvents();
          ...
          for (const change of changes) {
            switch (change.changeType) {
              ...
              case textModelEvents.RawContentChangedType.LinesInserted: {
                const insertedLineBreaks = lineBreaks.slice(
                  lineBreaksOffset,
  ...
  private _toModelVisibleRanges(visibleViewRange: Range): Range[] {
    const visibleRange =
      this.coordinatesConverter.convertViewRangeToModelRange(visibleViewRange);
  ...
  public setViewport(
    ...
    let position = this.coordinatesConverter.convertViewPositionToModelPosition(
      new Position(startLineNumber, this.getLineMinColumn(startLineNumber))
    );
    this._viewportStartLineTrackedRange = this.model._setTrackedRange(
      this._viewportStartLineTrackedRange,
      new Range(
    ...
    const viewportStartLineTop =
      this.viewLayout.getVerticalOffsetForLineNumber(startLineNumber);
  ...
  public getMinimapLinesRenderingData(
    ...
    let result = this._lines.getViewLinesData(
      startLineNumber,
      endLineNumber,
      needed
    );
  ...
  public getAllOverviewRulerDecorations(
    ...
    return this._lines.getAllOverviewRulerDecorations(
      this._editorId,
  ...
  public getPlainTextToCopy(
    ...
    if (!hasNonEmptyRange) {
      ...
      for (let i = 0; i < modelLineNumbers.length; i++) {
        ...
        result +=
          this.model.getLineContent(modelLineNumbers[i]) + newLineCharacter;
      }
      return result;
    }
  ...
  public getRichTextToCopy(
    ...
    const useDefaultFontFamily =
      hasBadChars || fontInfo.fontFamily === EDITOR_FONT_DEFAULTS.fontFamily;
