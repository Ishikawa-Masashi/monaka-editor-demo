...
class EditorOpener implements IOpener {
  ...
  async open(target: URI | string, options: OpenOptions) {
    ...
    await this._editorService.openCodeEditor(
      {
        resource: target as URI,
        options: {
          selection,
          context: options?.fromUserGesture
            ? EditorOpenContext.USER
            : EditorOpenContext.API,
    ...
    this._openers.push({
      open: async (target: URI | string, options?: OpenOptions) => {
        if (
          ...
          matchesScheme(target, Schemas.mailto) ||
          matchesScheme(target, Schemas.http) ||
          matchesScheme(target, Schemas.https)
        ) {
          // open externally
