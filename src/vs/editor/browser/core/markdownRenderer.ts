...
export class MarkdownRenderer {
  private static _ttpTokenizer = (window as any).trustedTypes?.createPolicy(
    'tokenizeToString',
    {
  ...
  protected _getRenderOptions(
    ...
    return {
      ...
      codeBlockRenderer: async (languageAlias, value) => {
        ...
        const tokenization =
          (await TokenizationRegistry.getPromise(modeId)) ?? undefined;
        ...
        element.innerHTML = (MarkdownRenderer._ttpTokenizer?.createHTML(
          value,
          tokenization
        ) ?? tokenizeToString(value, tokenization)) as string;
