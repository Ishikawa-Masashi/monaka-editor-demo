...
import {
  ...
  Direction,
} from '../../../../vs/editor/common/controller/cursorAtomicMoveOperations';
...
export class HitTestContext {
  ...
  public static getZoneAtCoord(
    ...
    const viewZoneWhitespace =
      context.viewLayout.getWhitespaceAtVerticalOffset(mouseVerticalOffset);
    ...
    if (viewZoneWhitespace) {
      ...
      if (viewZoneWhitespace.afterLineNumber > 0) {
        ...
        positionBefore = new Position(
          ...
          context.model.getLineMaxColumn(viewZoneWhitespace.afterLineNumber)
        );
      }
...
abstract class BareHitTestRequest {
  ...
  constructor(
    ...
    this.mouseVerticalOffset = Math.max(
      0,
      ctx.getCurrentScrollTop() + pos.y - editorPos.y
    );
...
class HitTestRequest extends BareHitTestRequest {
  ...
  public override toString(): string {
    return `pos(${this.pos.x},${this.pos.y}), editorPos(${this.editorPos.x},${
      this.editorPos.y
    }), mouseVerticalOffset: ${
...
export class MouseTargetFactory {
  ...
  private static _createMouseTarget(
    ...
    result =
      result || MouseTargetFactory._hitTestContentWidget(ctx, resolvedRequest);
    result =
      result || MouseTargetFactory._hitTestOverlayWidget(ctx, resolvedRequest);
    ...
    result =
      result || MouseTargetFactory._hitTestViewCursor(ctx, resolvedRequest);
    result =
      result || MouseTargetFactory._hitTestTextArea(ctx, resolvedRequest);
    result =
      result ||
      MouseTargetFactory._hitTestViewLines(
        ctx,
        resolvedRequest,
        domHitTestExecuted
      );
    result =
      result || MouseTargetFactory._hitTestScrollbar(ctx, resolvedRequest);
      ...
      if (ElementPath.isStrictChildOfViewLines(request.targetPath)) {
        ...
        if (request.mouseContentHorizontalOffset >= lineWidth) {
          ...
          const pos = new Position(
            lineNumber,
            ctx.model.getLineMaxColumn(lineNumber)
          );
          return request.fulfill(
            MouseTargetType.CONTENT_EMPTY,
            pos,
  ...
  private static _doHitTestWithCaretRangeFromPoint(
    ...
    const lineCenteredVerticalOffset =
  ...
  private static _actualDoHitTestWithCaretRangeFromPoint(
    ...
    const shadowRoot = dom.getShadowRoot(ctx.viewDomNode) as any;
    let range: Range;
    if (shadowRoot) {
    ...
    if (startContainer.nodeType === startContainer.TEXT_NODE) {
      ...
      if (parent3ClassName === ViewLine.CLASS_NAME) {
        const p = ctx.getPositionFromDOMInfo(
          <HTMLElement>parent1,
          range.startOffset
        );
        return {
...
export function shadowCaretRangeFromPoint(
