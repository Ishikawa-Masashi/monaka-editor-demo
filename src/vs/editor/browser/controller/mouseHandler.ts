...
export function createMouseMoveEventMerger(
  mouseTargetFactory: MouseTargetFactory | null
) {
  return function (
    lastEvent: EditorMouseEvent | null,
...
export class MouseHandler extends ViewEventHandler {
  ...
  constructor(
    ...
    const onMouseWheel = (browserEvent: IMouseWheelEvent) => {
      ...
      if (
        !this._context.configuration.options.get(EditorOption.mouseWheelZoom)
      ) {
        return;
      }
  ...
  private _getPositionOutsideEditor(e: EditorMouseEvent): MouseTarget | null {
    ...
    if (e.posy < editorContent.y) {
      const verticalOffset = Math.max(
        viewLayout.getCurrentScrollTop() - (editorContent.y - e.posy),
        0
      );
      ...
      if (viewZoneData) {
        ...
        if (newPosition) {
          return new MouseTarget(
            null,
            MouseTargetType.OUTSIDE_EDITOR,
            mouseColumn,
            newPosition
          );
        }
    ...
    if (e.posy > editorContent.y + editorContent.height) {
      const verticalOffset =
        viewLayout.getCurrentScrollTop() + (e.posy - editorContent.y);
      const viewZoneData = HitTestContext.getZoneAtCoord(
        this._context,
        verticalOffset
      );
