...
export abstract class EditorCommand extends Command {
  ...
  public static bindToContribution<T extends IEditorContribution>(
    controllerGetter: (editor: ICodeEditor) => T
  ...
  public runCommand(
    ...
    return editor.invokeWithinContext((editorAccessor) => {
      ...
      if (
        !kbService.contextMatchesRules(withNullAsUndefined(this.precondition))
      ) {
...
export abstract class EditorAction extends EditorCommand {
  ...
  protected reportTelemetry(accessor: ServicesAccessor, editor: ICodeEditor) {
    ...
    accessor
      ...
      .publicLog2<EditorActionInvokedEvent, EditorActionInvokedClassification>(
        'editorActionInvoked',
...
export function registerModelAndPositionCommand(
  id: string,
  handler: (model: ITextModel, position: Position, ...args: any[]) => any
) {
...
export function registerModelCommand(
  id: string,
  handler: (model: ITextModel, ...args: any[]) => any
) {
  CommandsRegistry.registerCommand(id, function (accessor, ...args) {
    const [resource] = args;
    assertType(URI.isUri(resource));
...
class EditorContributionRegistry {
  ...
  public registerEditorContribution<Services extends BrandedService[]>(
    ...
    this.editorContributions.push({
      id,
      ctor: ctor as IEditorContributionCtor,
    });
  }
...
registerCommand(
  new ProxyCommand(UndoCommand, { id: 'default:undo', precondition: undefined })
);
...
registerCommand(
  new ProxyCommand(RedoCommand, { id: 'default:redo', precondition: undefined })
);
