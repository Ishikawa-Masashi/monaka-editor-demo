...
} from '../../../../vs/editor/browser/viewParts/currentLineHighlight/currentLineHighlight';
...
export class View extends ViewEventHandler {
  ...
  // 追加したコード
  private _onDidRender2 = this._register(new Emitter<RenderingContext>());
  readonly onDidRender2 = this._onDidRender2.event;
  ...
  constructor(
    ...
    this._configPixelRatio = this._context.configuration.options.get(
  ...
  private _getEditorClassName() {
    ...
    return (
      this._context.configuration.options.get(EditorOption.editorClassName) +
      ' ' +
  ...
  private _actualRender(): void {
    ...
    // 追加したコード
    this._onDidRender2.fire(renderingContext);
  }
  ...
  public getOffsetForColumn(
    ...
    const viewPosition =
      this._context.model.coordinatesConverter.convertModelPositionToViewPosition(
        modelPosition
      );
    this._flushAccumulatedAndRenderNow();
  ...
  public layoutContentWidget(widgetData: IContentWidgetData): void {
    let newRange = widgetData.position
      ? widgetData.position.range || null
      : null;
    if (newRange === null) {
      const newPosition = widgetData.position
