...
class CSSBasedConfiguration extends Disposable {
  ...
  public readConfiguration(bareFontInfo: BareFontInfo): FontInfo {
    if (!this._cache.has(bareFontInfo)) {
      let readConfig =
        CSSBasedConfiguration._actualReadConfiguration(bareFontInfo);
  ...
  private static _actualReadConfiguration(
    ...
    const rightwardsArrow = this.createRequest(
      '→',
      CharWidthRequestType.Regular,
      all,
      monospace
    );
    const halfwidthRightwardsArrow = this.createRequest(
      '￫',
      CharWidthRequestType.Regular,
      all,
      null
    ...
    // U+00B7 - MIDDLE DOT
    const middot = this.createRequest(
      '·',
      CharWidthRequestType.Regular,
      all,
      monospace
    ...
    const wsmiddotWidth = this.createRequest(
...
export class Configuration extends CommonEditorConfiguration {
  ...
  constructor(
    ...
    this._register(
      CSSBasedConfiguration.INSTANCE.onDidChange(() => this._recomputeOptions())
    );
    ...
    this._register(
      browser.onDidChangeZoomLevel((_) => this._recomputeOptions())
    );
    this._register(
