  ...
  private _prepareRender(ctx: RenderingContext): ViewCursorRenderData | null {
    ...
    if (
      ...
      if (this._cursorStyle === TextEditorCursorStyle.Line) {
        ...
        if (width > 2) {
          ...
          const nextCharLength = strings.nextCharLength(
            lineContent,
            this._position.column - 1
          );
          textContent = lineContent.substr(
            this._position.column - 1,
            nextCharLength
          );
        }
    ...
    const visibleRangeForCharacter = ctx.linesVisibleRangesForRange(
      new Range(
        ...
        this._position.lineNumber,
        this._position.column + nextCharLength
      ),
      false
    );
    ...
    const width =
      range.width < 1 ? this._typicalHalfwidthCharacterWidth : range.width;

    let textContentClassName = '';
    if (this._cursorStyle === TextEditorCursorStyle.Block) {
      const lineData = this._context.model.getViewLineData(
        this._position.lineNumber
      );
      textContent = lineContent.substr(
        this._position.column - 1,
