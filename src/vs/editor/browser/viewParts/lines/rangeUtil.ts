...
export class RangeUtil {
  ...
  private static _createHorizontalRangesFromClientRects(
    ...
    mode = 'left-to-right-horizontal-writing'
  ): HorizontalRange[] | null {
    console.log('_createHorizontalRangesFromClientRects');
    ...
    if (mode === 'left-to-right-horizontal-writing') {
    ...
    if (mode === 'right-to-left-vertical-writing') {
      for (let i = 0, len = clientRects.length; i < len; i++) {
        const clientRect = clientRects[i];
        result[i] = new FloatHorizontalRange(
          Math.max(0, clientRect.top - clientRectDeltaLeft),
          clientRect.height
        );
      }
    }
