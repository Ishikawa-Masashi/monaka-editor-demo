...
import {
  ViewContext,
  EditorTheme,
} from '../../../../../vs/editor/common/view/viewContext';
...
class MinimapLayout {
  ...
  public static create(
    ...
    if (options.minimapHeightIsEditorHeight) {
      const logicalScrollHeight =
        ...
        (options.scrollBeyondLastLine
          ? viewportHeight - options.lineHeight
          : 0);
    ...
    if (minimapLinesFitting >= lineCount + extraLinesAtTheBottom) {
      ...
      const sliderTopAligned =
        ((viewportStartLineNumber - startLineNumber + partialLine) *
          minimapLineHeight) /
        pixelRatio;
...
class MinimapLine implements ILine {
  public viewLineNumber = undefined;
...
class MinimapBuffers {
  ...
  private static _createBackgroundFillData(
    ...
    // 後で修正すること
    const backgroundR = background ? background.r : 0;
    const backgroundG = background ? background.g : 0;
    const backgroundB = background ? background.b : 0;
  ...
  public getMinimapLinesRenderingData(
    ...
    if (this._samplingState) {
      ...
      for (
        ...
        if (needed[lineIndex]) {
          result[lineIndex] = this._context.model.getViewLineData(
    ...
    return this._context.model.getMinimapLinesRenderingData(
      startLineNumber,
      endLineNumber,
      needed
    ).data;
  ...
  public getMinimapDecorationsInViewport(
    ...
    if (this._samplingState) {
      ...
      visibleRange = new Range(
        ...
        1,
        modelEndLineNumber,
        this._context.model.getLineMaxColumn(modelEndLineNumber)
      );
    } else {
      visibleRange = new Range(
        startLineNumber,
    ...
    const decorations =
      this._context.model.getDecorationsInViewport(visibleRange);
...
class InnerMinimap extends Disposable {
  ...
  private _startSliderDragging(
    ...
    const handleMouseMove = (posy: number, posx: number) => {
      ...
      this._model.setScrollTop(
        initialSliderState.getDesiredScrollTopFromDelta(mouseDelta)
      );
    };
    ...
    this._sliderMouseMoveMonitor.startMonitoring(
      ...
      (mouseMoveData: IStandardMouseMoveEventData) =>
        handleMouseMove(mouseMoveData.posy, mouseMoveData.posx),
      () => {
        this._slider.toggleClassName('active', false);
      }
    );
  ...
  public render(renderingCtx: IMinimapRenderingContext): void {
    ...
    const scrollLeftChars =
      renderingCtx.scrollLeft /
      this._model.options.typicalHalfwidthCharacterWidth;
    const horizontalSliderLeft = Math.min(
      this._model.options.minimapWidth,
      Math.round(
        (scrollLeftChars * this._model.options.minimapCharWidth) /
          this._model.options.pixelRatio
      )
    );
    this._sliderHorizontal.setLeft(horizontalSliderLeft);
    this._sliderHorizontal.setWidth(
      this._model.options.minimapWidth - horizontalSliderLeft
    );
  ...
  private renderDecorations(layout: MinimapLayout) {
    if (this._renderDecorations) {
      ...
      for (let i = 0; i < decorations.length; i++) {
        ...
        const decorationColor = (<ModelDecorationMinimapOptions>(
          decoration.options.minimap
        )).getColor(this._theme);
        for (
          let line = decoration.range.startLineNumber;
  ...
  private renderDecorationOnLine(
    ...
    const { startColumn, endColumn, startLineNumber, endLineNumber } =
      decorationRange;
    const x =
...
registerThemingParticipant((theme, collector) => {
  ...
  if (minimapBackgroundValue) {
    collector.addRule(
      `.monaco-editor .minimap > canvas { opacity: ${minimapBackgroundValue.rgba.a}; will-change: opacity; }`
    );
  }
  const sliderBackground = theme.getColor(minimapSliderBackground);
  if (sliderBackground) {
    collector.addRule(
      `.monaco-editor .minimap-slider .minimap-slider-horizontal { background: ${sliderBackground}; }`
    );
  }
  const sliderHoverBackground = theme.getColor(minimapSliderHoverBackground);
  if (sliderHoverBackground) {
    collector.addRule(
      `.monaco-editor .minimap-slider:hover .minimap-slider-horizontal { background: ${sliderHoverBackground}; }`
    );
  }
  const sliderActiveBackground = theme.getColor(minimapSliderActiveBackground);
  if (sliderActiveBackground) {
    collector.addRule(
      `.monaco-editor .minimap-slider.active .minimap-slider-horizontal { background: ${sliderActiveBackground}; }`
    );
  }
