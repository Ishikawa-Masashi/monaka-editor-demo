...
export abstract class AbstractLineHighlightOverlay extends DynamicViewOverlay {
  ...
  private _readFromSelections(): boolean {
    ...
    const renderSelections = isRenderedUsingBorder
      ? this._selections.slice(0, 1)
      : this._selections;

    const cursorsLineNumbers = renderSelections.map(
      (s) => s.positionLineNumber
    );
...
export class CurrentLineMarginHighlightOverlay extends AbstractLineHighlightOverlay {
  protected _renderOne(ctx: RenderingContext): string {
    const className =
      'current-line' +
      (this._shouldRenderMargin() ? ' current-line-margin' : '') +
