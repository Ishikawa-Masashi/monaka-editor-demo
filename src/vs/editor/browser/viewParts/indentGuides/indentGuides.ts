...
export class IndentGuidesOverlay extends DynamicViewOverlay {
  ...
  public override onCursorStateChanged(
    ...
    const newPrimaryLineNumber = selection.isEmpty()
  ...
  public prepareRender(ctx: RenderingContext): void {
    ...
    //   const activeIndentInfo = this._context.model.getActiveIndentGuide(
...
registerThemingParticipant((theme, collector) => {
  ...
  if (editorIndentGuidesColor) {
    collector.addRule(
      `.monaco-editor .lines-content .cigr { box-shadow: 1px 0 0 0 ${editorIndentGuidesColor} inset; }`
    );
  }
  const editorActiveIndentGuidesColor =
    theme.getColor(editorActiveIndentGuides) || editorIndentGuidesColor;
  if (editorActiveIndentGuidesColor) {
    collector.addRule(
      `.monaco-editor .lines-content .cigra { box-shadow: 1px 0 0 0 ${editorActiveIndentGuidesColor} inset; }`
    );
  }
});
