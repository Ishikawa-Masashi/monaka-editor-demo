...
const diffReviewInsertIcon = registerIcon(
  'diff-review-insert',
  Codicon.add,
  nls.localize('diffReviewInsertIcon', "Icon for 'Insert' in diff review.")
);
const diffReviewRemoveIcon = registerIcon(
  'diff-review-remove',
  Codicon.remove,
  nls.localize('diffReviewRemoveIcon', "Icon for 'Remove' in diff review.")
);
const diffReviewCloseIcon = registerIcon(
  'diff-review-close',
  Codicon.close,
  nls.localize('diffReviewCloseIcon', "Icon for 'Close' in diff review.")
);

export class DiffReview extends Disposable {
  private static _ttPolicy = (window as any).trustedTypes?.createPolicy(
    'diffReview',
    {
  ...
  constructor(diffEditor: DiffEditorWidget) {
    ...
    this._actionBar = this._register(
      new ActionBar(this.actionBarContainer.domNode)
    );
    ...
    this.scrollbar = this._register(
      new DomScrollableElement(this._content.domNode, {})
    );
    ...
    this._register(
      dom.addStandardDisposableListener(this.domNode.domNode, 'click', (e) => {
        e.preventDefault();
  ...
  private static _mergeAdjacent(
    ...
    for (let i = 0, len = lineChanges.length; i < len; i++) {
      ...
      // Emit before anchors
      {
        const originalEqualAbove =
          originalEnd === 0 ? originalStart : originalStart - 1;
        const modifiedEqualAbove =
      ...
      // Emit after anchors
      {
        const originalEqualBelow =
          originalEnd === 0 ? originalStart + 1 : originalEnd + 1;
        const modifiedEqualBelow =
  ...
  private _render(): void {
    ...
    Configuration.applyFontInfoSlow(
      container,
      modifiedOptions.get(EditorOption.fontInfo)
    );
    ...
    cell.appendChild(
      document.createTextNode(
        `${diffIndex + 1}/${
          this._diffs.length
        }: @@ -${minOriginalLine},${originalChangedLinesCnt} +${minModifiedLine},${modifiedChangedLinesCnt} @@`
      )
    );
  ...
  private static _renderSection(
    ...
    for (let i = 0; i <= cnt; i++) {
      ...
      if (modifiedLine !== 0) {
        let html: string | any = this._renderLine(
          modifiedModel,
          modifiedOptions,
...
registerThemingParticipant((theme, collector) => {
  ...
  if (lineNumbers) {
    collector.addRule(
      `.monaco-diff-editor .diff-review-line-number { color: ${lineNumbers}; }`
    );
  }
  ...
  if (shadow) {
    collector.addRule(
      `.monaco-diff-editor .diff-review-shadow { box-shadow: ${shadow} 0 -6px 6px -6px inset; }`
    );
  }
});
...
function findFocusedDiffEditor(
  accessor: ServicesAccessor
): DiffEditorWidget | null {
  ...
  for (let i = 0, len = diffEditors.length; i < len; i++) {
    ...
    if (
      diffEditor.getModifiedEditor().getId() === activeCodeEditor.getId() ||
