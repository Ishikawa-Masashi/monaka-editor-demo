...
} from '../../../../vs/editor/common/services/editorWorkerService';
...
import {
  IDiffLinesChange,
  InlineDiffMargin,
} from '../../../../vs/editor/browser/widget/inlineDiffMargin';
...
class VisualEditorState {
  ...
  public apply(
    ...
    overviewRuler: editorBrowser.IOverviewRuler | null,
    newDecorations: IEditorDiffDecorationsWithZones,
    restoreScrollState: boolean
  ): void {
    const scrollState = restoreScrollState
  ...
  implements editorBrowser.IDiffEditor
{
  ...
  constructor(
    ...
    this._originalIsEditable = validateBooleanOption(
    ...
    this._elementSizeObserver = this._register(
      new ElementSizeObserver(
        ...
        options.dimension,
        () => this._onDidContainerSizeChanged()
      )
    );
    ...
    this._containerDomElement.appendChild(
    ...
    this._register(
      themeService.onDidColorThemeChange((t) => {
  ...
  private _recreateOverviewRulers(): void {
    ...
    if (this._originalEditor.hasModel()) {
      this._originalOverviewRuler = this._originalEditor.createOverviewRuler(
  ...
  private _createLeftHandSideEditor(
    ...
    const editor = this._createInnerEditor(
      ...
      this._adjustOptionsForLeftHandSide(options),
    ...
    this._register(
      editor.onDidContentSizeChange((e) => {
        const width =
          this._originalEditor.getContentWidth() +
          this._modifiedEditor.getContentWidth() +
          DiffEditorWidget.ONE_OVERVIEW_WIDTH;
        const height = Math.max(
          this._modifiedEditor.getContentHeight(),
          this._originalEditor.getContentHeight()
        );
  ...
  protected _createInnerEditor(
    ...
    options: Readonly<editorBrowser.IEditorConstructionOptions>,
  ...
  public updateOptions(newOptions: Readonly<IDiffEditorOptions>): void {
    ...
    this._originalIsEditable = validateBooleanOption(
      newOptions.originalEditable,
      this._originalIsEditable
    );
    this._diffCodeLens = validateBooleanOption(
      newOptions.diffCodeLens,
      this._diffCodeLens
    );
    this._diffWordWrap = validateDiffWordWrap(
      newOptions.diffWordWrap,
      this._diffWordWrap
    );
    ...
    this._originalEditor.updateOptions(
      this._adjustOptionsForLeftHandSide(newOptions)
    );
    ...
    // renderOverviewRuler
    if (
      typeof newOptions.renderOverviewRuler !== 'undefined' &&
  ...
  public revealLineInCenterIfOutsideViewport(
    ...
    this._modifiedEditor.revealLineInCenterIfOutsideViewport(
      lineNumber,
      scrollType
    );
  }
  ...
  public revealPositionInCenterIfOutsideViewport(
    ...
    this._modifiedEditor.revealPositionInCenterIfOutsideViewport(
      position,
      scrollType
    );
  }
  ...
  public revealLinesInCenter(
    ...
    this._modifiedEditor.revealLinesInCenter(
      startLineNumber,
      endLineNumber,
      scrollType
    );
  ...
  public revealLinesInCenterIfOutsideViewport(
    ...
    this._modifiedEditor.revealLinesInCenterIfOutsideViewport(
      startLineNumber,
      endLineNumber,
      scrollType
    );
  ...
  public revealLinesNearTop(
    ...
    this._modifiedEditor.revealLinesNearTop(
      startLineNumber,
      endLineNumber,
      scrollType
    );
  ...
  public revealRange(
    ...
    this._modifiedEditor.revealRange(
      range,
      scrollType,
  ...
  public revealRangeInCenterIfOutsideViewport(
    ...
    this._modifiedEditor.revealRangeInCenterIfOutsideViewport(
      range,
      scrollType
    );
  }
  ...
  public hasTextFocus(): boolean {
    return (
      this._originalEditor.hasTextFocus() || this._modifiedEditor.hasTextFocus()
    );
  }
  ...
  private _updateDecorations(): void {
    ...
    const lineChanges = this._diffComputationResult
      ? this._diffComputationResult.changes
      : [];

    const foreignOriginal = this._originalEditorState.getForeignViewZones(
      this._originalEditor.getWhitespaces()
    );
    const foreignModified = this._modifiedEditorState.getForeignViewZones(
      this._modifiedEditor.getWhitespaces()
    );

    const diffDecorations = this._strategy.getEditorsDiffDecorations(
      lineChanges,
      this._ignoreTrimWhitespace,
  ...
  private _adjustOptionsForSubEditor(
    options: Readonly<editorBrowser.IDiffEditorConstructionOptions>
  ): editorBrowser.IEditorConstructionOptions {
  ...
  private _computeOverviewViewport(): { height: number; top: number } | null {
    ...
    const computedSliderSize = Math.max(
      0,
  ...
  private _getEquivalentLineForOriginalLineNumber(lineNumber: number): number {
    const lineChange = this._getLineChangeAtOrBeforeLineNumber(
      lineNumber,
    ...
    const originalEquivalentLineNumber =
      lineChange.originalStartLineNumber +
      (lineChange.originalEndLineNumber > 0 ? -1 : 0);
    const modifiedEquivalentLineNumber =
      lineChange.modifiedStartLineNumber +
      (lineChange.modifiedEndLineNumber > 0 ? -1 : 0);
    const lineChangeOriginalLength =
      lineChange.originalEndLineNumber > 0
        ? lineChange.originalEndLineNumber -
          lineChange.originalStartLineNumber +
          1
        : 0;
    const lineChangeModifiedLength =
  ...
  public getDiffLineInformationForOriginal(
    lineNumber: number
  ): editorBrowser.IDiffLineInformation | null {
    if (!this._diffComputationResult) {
    ...
    return {
      equivalentLineNumber:
        this._getEquivalentLineForOriginalLineNumber(lineNumber),
    };
  }
  ...
  public getDiffLineInformationForModified(
    ...
    return {
      equivalentLineNumber:
        this._getEquivalentLineForModifiedLineNumber(lineNumber),
    };
  }
}
...
abstract class DiffEditorWidgetStyle extends Disposable {
  ...
  public getEditorsDiffDecorations(
    ...
    const zones = this._getViewZones(
      lineChanges,
      originalWhitespaces,
      modifiedWhitespaces,
      renderIndicators
    ...
    const originalDecorations = this._getOriginalEditorDecorations(
      lineChanges,
      ignoreTrimWhitespace,
      renderIndicators
    );
    const modifiedDecorations = this._getModifiedEditorDecorations(
      lineChanges,
      ignoreTrimWhitespace,
      renderIndicators
    );
  ...
  protected abstract _getViewZones(
    lineChanges: editorCommon.ILineChange[],
    originalForeignVZ: IEditorWhitespace[],
    modifiedForeignVZ: IEditorWhitespace[],
    renderIndicators: boolean
  ): IEditorsZones;
  protected abstract _getOriginalEditorDecorations(
    lineChanges: editorCommon.ILineChange[],
    ignoreTrimWhitespace: boolean,
    renderIndicators: boolean
  ): IEditorDiffDecorations;
  protected abstract _getModifiedEditorDecorations(
    lineChanges: editorCommon.ILineChange[],
    ignoreTrimWhitespace: boolean,
    renderIndicators: boolean
  ): IEditorDiffDecorations;

  public abstract setEnableSplitViewResizing(
...
abstract class ViewZonesComputer {
  ...
  private static _getViewLineCount(
    editor: CodeEditorWidget,
    startLineNumber: number,
    endLineNumber: number
  ): number {
  ...
  public getViewZones(): IEditorsZones {
    ...
    const originalHasWrapping =
      this._originalEditor.getOption(EditorOption.wrappingInfo)
        .wrappingColumn !== -1;
    const modifiedHasWrapping =
      this._modifiedEditor.getOption(EditorOption.wrappingInfo)
        .wrappingColumn !== -1;
    const hasWrapping = originalHasWrapping || modifiedHasWrapping;
    const originalModel = this._originalEditor.getModel()!;
    const originalCoordinatesConverter =
      this._originalEditor._getViewModel()!.coordinatesConverter;
    const modifiedCoordinatesConverter =
      this._modifiedEditor._getViewModel()!.coordinatesConverter;
    ...
    const addAndCombineIfPossible = (
      destination: IMyViewZone[],
      item: IMyViewZone
    ) => {
    ...
    const modifiedForeignVZ = new ForeignViewZonesIterator(
      this._modifiedForeignVZ
    );
    const originalForeignVZ = new ForeignViewZonesIterator(
      this._originalForeignVZ
    );
    ...
    for (let i = 0, length = this._lineChanges.length; i <= length; i++) {
      ...
      if (lineChange !== null) {
        ...
        lineChangeOriginalLength =
          lineChange.originalEndLineNumber > 0
            ? ViewZonesComputer._getViewLineCount(
                this._originalEditor,
                lineChange.originalStartLineNumber,
                lineChange.originalEndLineNumber
              )
      ...
      if (hasWrapping) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const originalViewLineCount =
            originalCoordinatesConverter.getModelLineViewLineCount(
              originalLineNumber
            );
          const modifiedViewLineCount =
            modifiedCoordinatesConverter.getModelLineViewLineCount(
              modifiedLineNumber
            );
      ...
      while (
        ...
        if (
          ...
          marginDomNode =
            this._createOriginalMarginDomNodeForModifiedForeignViewZoneInAddedRegion();
        }
      ...
      if (lineChange !== null && isChangeOrInsert(lineChange)) {
        const r = this._produceOriginalFromDiff(
          lineChange,
          lineChangeOriginalLength,
          lineChangeModifiedLength
        );
      ...
      if (lineChange !== null && isChangeOrDelete(lineChange)) {
        const r = this._produceModifiedFromDiff(
          lineChange,
          lineChangeOriginalLength,
          lineChangeModifiedLength
        );
...
class DiffEditorWidgetSideBySide
  extends DiffEditorWidgetStyle
  implements IVerticalSashLayoutProvider
{
  ...
  public setEnableSplitViewResizing(enableSplitViewResizing: boolean): void {
    ...
    if (this._disableSash !== newDisableSash) {
      ...
      this._sash.state = this._disableSash
        ? SashState.Disabled
        : SashState.Enabled;
    }
  }
  ...
  public layout(sashRatio: number | null = this._sashRatio): number {
    ...
    const contentWidth =
      w -
      (this._dataSource.getOptions().renderOverviewRuler
        ? DiffEditorWidget.ENTIRE_DIFF_OVERVIEW_WIDTH
        : 0);
  ...
  private _onSashDrag(e: ISashEvent): void {
    ...
    const contentWidth =
      ...
      (this._dataSource.getOptions().renderOverviewRuler
        ? DiffEditorWidget.ENTIRE_DIFF_OVERVIEW_WIDTH
        : 0);
    const sashPosition = this.layout(
  ...
  protected _getViewZones(
    ...
    const c = new SideBySideViewZonesComputer(
      lineChanges,
      originalForeignVZ,
      modifiedForeignVZ,
      originalEditor,
  ...
  protected _getOriginalEditorDecorations(
    lineChanges: editorCommon.ILineChange[],
    ignoreTrimWhitespace: boolean,
    renderIndicators: boolean
  ): IEditorDiffDecorations {
              ...
              if (ignoreTrimWhitespace) {
                for (
                  ...
                  if (lineNumber === charChange.originalStartLineNumber) {
                    ...
                    startColumn =
                      originalModel.getLineFirstNonWhitespaceColumn(lineNumber);
                  }
                  if (lineNumber === charChange.originalEndLineNumber) {
                    endColumn = charChange.originalEndColumn;
                  } else {
                    endColumn =
                      originalModel.getLineLastNonWhitespaceColumn(lineNumber);
                  }
                  result.decorations.push(
                    createDecoration(
                      lineNumber,
  ...
  protected _getModifiedEditorDecorations(
    lineChanges: editorCommon.ILineChange[],
    ignoreTrimWhitespace: boolean,
    renderIndicators: boolean
  ): IEditorDiffDecorations {
              ...
              if (ignoreTrimWhitespace) {
                for (
                  ...
                  if (lineNumber === charChange.modifiedStartLineNumber) {
                    ...
                    startColumn =
                      modifiedModel.getLineFirstNonWhitespaceColumn(lineNumber);
                  }
                  if (lineNumber === charChange.modifiedEndLineNumber) {
                    endColumn = charChange.modifiedEndColumn;
                  } else {
                    endColumn =
                      modifiedModel.getLineLastNonWhitespaceColumn(lineNumber);
                  }
                  result.decorations.push(
                    createDecoration(
                      lineNumber,
...
class SideBySideViewZonesComputer extends ViewZonesComputer {
  ...
  protected _produceOriginalFromDiff(
    lineChange: editorCommon.ILineChange,
    lineChangeOriginalLength: number,
    lineChangeModifiedLength: number
  ): IMyViewZone | null {
  ...
  protected _produceModifiedFromDiff(
    lineChange: editorCommon.ILineChange,
    lineChangeOriginalLength: number,
    lineChangeModifiedLength: number
  ): IMyViewZone | null {
...
class DiffEditorWidgetInline extends DiffEditorWidgetStyle {
  ...
  constructor(dataSource: IDataSource, enableSplitViewResizing: boolean) {
    ...
    this._decorationsLeft = dataSource
      .getOriginalEditor()
    ...
    this._register(
      dataSource
        .getOriginalEditor()
        .onDidLayoutChange((layoutInfo: EditorLayoutInfo) => {
  ...
  protected _getViewZones(
    ...
    const originalEditor = this._dataSource.getOriginalEditor();
    const modifiedEditor = this._dataSource.getModifiedEditor();
    const computer = new InlineViewZonesComputer(
      lineChanges,
      originalForeignVZ,
      modifiedForeignVZ,
      originalEditor,
...
class InlineViewZonesComputer extends ViewZonesComputer {
  ...
  constructor(
    ...
    this._lineBreaksComputer = this._modifiedEditor
      ._getViewModel()!
  ...
  protected _produceModifiedFromDiff(
    ...
    for (
      ...
      this._lineBreaksComputer.addRequest(
        this._originalModel.getLineContent(lineNumber),
        null
      );
    }
  ...
  private _finalize(result: IEditorsZones): void {
    ...
    const mightContainNonBasicASCII =
      this._originalModel.mightContainNonBasicASCII();
      ...
      for (
        ...
        if (lineBreakData) {
          ...
          for (const breakOffset of lineBreakData.breakOffsets) {
            ...
            const viewLineContent = lineContent.substring(
              lastBreakOffset,
              breakOffset
            );
  ...
  private _renderOriginalLine(
    ...
    if (this._renderIndicators) {
      ...
      marginElement.className = `delete-sign ${ThemeIcon.asClassName(
        diffRemoveIcon
      )}`;
      marginElement.setAttribute(
        'style',
        `position:absolute;top:${
          renderedLineCount * lineHeight
        }px;width:${lineDecorationsWidth}px;height:${lineHeight}px;right:0;`
      );
...
function validateDiffWordWrap(
  value: 'off' | 'on' | 'inherit' | undefined,
  defaultValue: 'off' | 'on' | 'inherit'
): 'off' | 'on' | 'inherit' {
  return validateStringSetOption<'off' | 'on' | 'inherit'>(
    value,
    defaultValue,
    ['off', 'on', 'inherit']
  );
...
function getViewRange(
  ...
  return viewModel.coordinatesConverter.convertModelRangeToViewRange(
    new Range(
      startLineNumber,
      model.getLineMinColumn(startLineNumber),
      endLineNumber,
...
registerThemingParticipant((theme, collector) => {
  ...
  if (added) {
    ...
    collector.addRule(
      `.monaco-diff-editor .line-insert, .monaco-diff-editor .char-insert { background-color: ${added}; }`
    );
    collector.addRule(
  ...
  if (removed) {
    ...
    collector.addRule(
      `.monaco-diff-editor .line-delete, .monaco-diff-editor .char-delete { background-color: ${removed}; }`
    );
    collector.addRule(
  ...
  if (border) {
    collector.addRule(
      `.monaco-diff-editor.side-by-side .editor.modified { border-left: 1px solid ${border}; }`
    );
  }
