...
export class InlineDiffMargin extends Disposable {
  ...
  constructor(
    ...
    this._diffActions.className =
      Codicon.lightBulb.classNames + ' lightbulb-glyph';
    ...
    if (diff.originalEndLineNumber > diff.modifiedStartLineNumber) {
      copyLineAction = new Action(
        ...
        true,
        async () => {
          const lineContent = diff.originalModel.getLineContent(
            diff.originalStartLineNumber + currentLineNumberOffset
          );
    ...
    if (!readOnly) {
      actions.push(
        new Action(
          ...
          nls.localize('diff.inline.revertChange.label', 'Revert this change'),
          undefined,
          true,
          async () => {
            const range = new Range(
              ...
              diff.originalModel.getLineMaxColumn(diff.originalEndLineNumber)
            );
            const deletedText = diff.originalModel.getValueInRange(range);
            if (diff.modifiedEndLineNumber === 0) {
              ...
              const column = editor
                .getModel()!
                .getLineMaxColumn(diff.modifiedStartLineNumber);
              ...
              const column = editor
                .getModel()!
                .getLineMaxColumn(diff.modifiedEndLineNumber);
              editor.executeEdits('diffEditor', [
                {
                  range: new Range(
                    diff.modifiedStartLineNumber,
    ...
    const showContextMenu = (x: number, y: number) => {
      this._contextMenuService.showContextMenu({
        ...
        getActions: () => {
          if (copyLineAction) {
            copyLineAction.label = nls.localize(
              'diff.clipboard.copyDeletedLineContent.label',
              'Copy deleted line ({0})',
              diff.originalStartLineNumber + currentLineNumberOffset
            );
    ...
    this._register(
      dom.addStandardDisposableListener(this._diffActions, 'mousedown', (e) => {
