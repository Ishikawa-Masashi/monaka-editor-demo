  ...
  implements editorBrowser.ICodeEditor
{
  ...
  private readonly _onDidLayoutChange: Emitter<EditorLayoutInfo> =
    this._register(new Emitter<EditorLayoutInfo>());
  ...
  private readonly _onDidPaste: Emitter<editorBrowser.IPasteEvent> =
    this._register(new Emitter<editorBrowser.IPasteEvent>());
  ...
  private readonly _onMouseDrop: Emitter<editorBrowser.IPartialEditorMouseEvent> =
    this._register(new Emitter<editorBrowser.IPartialEditorMouseEvent>());
  ...
  protected _onDidRender = this._register(new Emitter<RenderingContext>());
  ...
  constructor(
    ...
    console.log(`${this._telemetryData ? 'true' : 'false'}`);

    this._configuration = this._register(
      this._createConfiguration(options, accessibilityService)
    );
    this._register(
    ...
    for (const desc of contributions) {
      if (this._contributions[desc.id]) {
        onUnexpectedError(
          new Error(`Cannot have two contributions with the same id ${desc.id}`)
        );
        continue;
      }
      try {
        const contribution = this._instantiationService.createInstance(
          desc.ctor,
          this
        );
    ...
    EditorExtensionsRegistry.getEditorActions().forEach((action) => {
      if (this._actions[action.id]) {
        onUnexpectedError(
          new Error(`Cannot have two actions with the same id ${action.id}`)
        );
        return;
      }
        ...
        (): Promise<void> => {
          return this._instantiationService.invokeFunction((accessor) => {
            return Promise.resolve(
              action.runEditorCommand(accessor, this, null)
            );
          });
        },
  ...
  public setModel(
    _model: ITextModel | editorCommon.IDiffEditorModel | null = null
  ): void {
  ...
  private _paste(
    ...
    const startPosition = this._modelData.viewModel
      .getSelection()
      .getStartPosition();
    this._modelData.viewModel.paste(
      text,
      pasteOnNewLine,
      multicursorText,
      source
    );
    const endPosition = this._modelData.viewModel
      .getSelection()
      .getStartPosition();
    if (source === 'keyboard') {
  ...
  private _triggerEditorCommand(
    ...
    if (command) {
      ...
      this._instantiationService.invokeFunction((accessor) => {
        Promise.resolve(command.runEditorCommand(accessor, this, payload)).then(
          undefined,
          onUnexpectedError
        );
      });
  ...
  protected _attachModel(model: ITextModel | null): void {
    ...
    listenersToRemove.push(
      model.onDidChangeLanguageConfiguration((e) =>
        this._onDidChangeModelLanguageConfiguration.fire(e)
      )
    );
    listenersToRemove.push(
      model.onDidChangeContent((e) => this._onDidChangeModelContent.fire(e))
    );
    listenersToRemove.push(
      model.onDidChangeOptions((e) => this._onDidChangeModelOptions.fire(e))
    );
  ...
  protected _createView(viewModel: ViewModel): [View, boolean] {
    ...
    if (this.isSimpleWidget) {
      ...
      commandDelegate = {
        paste: (
          ...
          const payload: editorCommon.PastePayload = {
            text,
            pasteOnNewLine,
            multicursorText,
            mode,
    ...
    view.onDidRender2((evt) => {
      this._onDidRender.fire(evt);
    });
...
class EditorContextKeysManager extends Disposable {
  ...
  constructor(editor: CodeEditorWidget, contextKeyService: IContextKeyService) {
    ...
    this._register(
      this._editor.onDidChangeConfiguration(() => this._updateFromModel())
    );
...
export class EditorModeContext extends Disposable {
  ...
  constructor(
    ...
    this._isInWalkThrough =
      EditorContextKeys.isInWalkThroughSnippet.bindTo(_contextKeyService);
  ...
  private _update() {
    ...
    this._contextKeyService.bufferChangeEvents(() => {
      ...
      this._hasInlayHintsProvider.set(
        modes.InlayHintsProviderRegistry.has(model)
      );
...
registerThemingParticipant((theme, collector) => {
  ...
  if (errorBorderColor) {
    collector.addRule(
      `.monaco-editor .${ClassName.EditorErrorDecoration} { border-bottom: 4px double ${errorBorderColor}; }`
    );
  }
  ...
  if (errorBackground) {
    collector.addRule(
      `.monaco-editor .${ClassName.EditorErrorDecoration}::before { display: block; content: ''; width: 100%; height: 100%; background: ${errorBackground}; }`
    );
  }
  ...
  if (warningBorderColor) {
    collector.addRule(
      `.monaco-editor .${ClassName.EditorWarningDecoration} { border-bottom: 4px double ${warningBorderColor}; }`
    );
  }
  ...
  if (warningBackground) {
    collector.addRule(
      `.monaco-editor .${ClassName.EditorWarningDecoration}::before { display: block; content: ''; width: 100%; height: 100%; background: ${warningBackground}; }`
    );
  }
  ...
  if (infoBorderColor) {
    collector.addRule(
      `.monaco-editor .${ClassName.EditorInfoDecoration} { border-bottom: 4px double ${infoBorderColor}; }`
    );
  }
  ...
  if (infoBackground) {
    collector.addRule(
      `.monaco-editor .${ClassName.EditorInfoDecoration}::before { display: block; content: ''; width: 100%; height: 100%; background: ${infoBackground}; }`
    );
  }
  ...
  if (hintBorderColor) {
    collector.addRule(
      `.monaco-editor .${ClassName.EditorHintDecoration} { border-bottom: 2px dotted ${hintBorderColor}; }`
    );
  }
  ...
  if (unnecessaryBorder) {
    collector.addRule(
      `.monaco-editor.showUnused .${ClassName.EditorUnnecessaryDecoration} { border-bottom: 2px dashed ${unnecessaryBorder}; }`
    );
  }

  const deprecatedForeground = theme.getColor(editorForeground) || 'inherit';
  collector.addRule(
    `.monaco-editor.showDeprecated .${ClassName.EditorDeprecatedInlineDecoration} { text-decoration: line-through; text-decoration-color: ${deprecatedForeground}}`
  );
});
