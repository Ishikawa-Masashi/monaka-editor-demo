...
import {
  DiffNavigator,
  IDiffNavigator,
} from '../../../../vs/editor/browser/widget/diffNavigator';
import {
  EditorOptions,
...
} from '../../../../vs/editor/standalone/common/standaloneThemeService';
import {
  CommandsRegistry,
  ICommandService,
...
function withAllStandaloneServices<T extends IEditor>(
...
export function colorizeElement(
  ...
  const themeService = <StandaloneThemeServiceImpl>(
...
function getSafeTokenizationSupport(
  language: string
): Omit<modes.ITokenizationSupport, 'tokenize2'> {
  ...
  return {
    ...
    tokenize: (
      ...
      state: modes.IState,
      deltaOffset: number
    ) => nullTokenize(language, line, state, deltaOffset),
  };
}
