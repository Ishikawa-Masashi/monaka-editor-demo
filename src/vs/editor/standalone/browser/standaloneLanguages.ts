...
export class TokenizationSupport2Adapter implements modes.ITokenizationSupport {
  ...
  public static adaptTokenize(
    language: string,
    actual: { tokenize(line: string, state: modes.IState): ILineTokens },
    line: string,
    state: modes.IState,
    offsetDelta: number
  ): TokenizationResult {
    let actualResult = actual.tokenize(line, state);
    let tokens = TokenizationSupport2Adapter._toClassicTokens(
      actualResult.tokens,
      language,
      offsetDelta
    );
...
export function setTokensProvider(
  ...
  const create = (provider: TokensProvider | EncodedTokensProvider) => {
    if (isEncodedTokensProvider(provider)) {
      return new EncodedTokenizationSupport2Adapter(
        languageIdentifier!,
        provider
      );
    } else {
...
export function registerHoverProvider(
  ...
  return modes.HoverProviderRegistry.register(languageId, {
    provideHover: (
      ...
      return Promise.resolve<modes.Hover | null | undefined>(
        provider.provideHover(model, position, token)
      ).then((value): modes.Hover | undefined => {
        if (!value) {
          return undefined;
        }
        if (!value.range && word) {
...
export function registerLinkedEditingRangeProvider(
  languageId: string,
  provider: modes.LinkedEditingRangeProvider
): IDisposable {
  return modes.LinkedEditingRangeProviderRegistry.register(
    languageId,
    provider
  );
}
...
export function registerCodeActionProvider(
  ...
  return modes.CodeActionProviderRegistry.register(languageId, {
    provideCodeActions: (
      ...
      token: CancellationToken
    ): modes.ProviderResult<modes.CodeActionList> => {
...
export function registerColorProvider(
  languageId: string,
  provider: modes.DocumentColorProvider
): IDisposable {
...
export function registerFoldingRangeProvider(
  languageId: string,
  provider: modes.FoldingRangeProvider
): IDisposable {
...
export function registerDeclarationProvider(
  languageId: string,
  provider: modes.DeclarationProvider
): IDisposable {
...
export function registerSelectionRangeProvider(
  languageId: string,
  provider: modes.SelectionRangeProvider
): IDisposable {
...
export function registerDocumentSemanticTokensProvider(
  languageId: string,
  provider: modes.DocumentSemanticTokensProvider
): IDisposable {
  return modes.DocumentSemanticTokensProviderRegistry.register(
    languageId,
    provider
  );
}
...
export function registerDocumentRangeSemanticTokensProvider(
  languageId: string,
  provider: modes.DocumentRangeSemanticTokensProvider
): IDisposable {
  return modes.DocumentRangeSemanticTokensProviderRegistry.register(
    languageId,
    provider
  );
}
...
export function registerInlineCompletionsProvider(
  languageId: string,
  provider: modes.InlineCompletionsProvider
): IDisposable {
