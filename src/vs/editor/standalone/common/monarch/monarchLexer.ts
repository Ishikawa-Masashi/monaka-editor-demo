...
class MonarchClassicTokensCollector implements IMonarchTokensCollector {
  ...
  public nestedModeTokenize(
    ...
    const nestedModeTokenizationSupport =
      modes.TokenizationRegistry.get(nestedModeId);
...
class MonarchModernTokensCollector implements IMonarchTokensCollector {
  ...
  public enterMode(startOffset: number, modeId: string): void {
    this._currentLanguageId =
      this._modeService.getLanguageIdentifier(modeId)!.id;
  }
...
export class MonarchTokenizer implements modes.ITokenizationSupport {
  ...
  public getLoadStatus(): ILoadStatus {
    ...
    for (let nestedModeId in this._embeddedModes) {
      ...
      const tokenizationSupportPromise =
        modes.TokenizationRegistry.getPromise(nestedModeId);
  ...
  private _myTokenize(
    ...
    const line =
      hasEOL && this._lexer.includeLF ? lineWithoutLF + '\n' : lineWithoutLF;
    ...
    while (forceEvaluation || pos < lineLength) {
      ...
      const computeNewStateForEmbeddedMode = (enteringEmbeddedMode: string) => {
        ...
        let enteringEmbeddedModeId =
          this._modeService.getModeIdForLanguageName(enteringEmbeddedMode);
        if (enteringEmbeddedModeId) {
          enteringEmbeddedMode = enteringEmbeddedModeId;
        }

        const embeddedModeData =
          this._getNestedEmbeddedModeData(enteringEmbeddedMode);
