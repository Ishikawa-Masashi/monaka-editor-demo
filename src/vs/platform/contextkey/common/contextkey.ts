...
export abstract class ContextKeyExpr {
  ...
  private static _deserializeRegexValue(
    ...
    if (start === end || start < 0 /* || to < 0 */) {
      if (strict) {
        throw new Error(
          `bad regexp-value '${serializedValue}', missing /-enclosure`
        );
      } else {
        console.warn(
          `bad regexp-value '${serializedValue}', missing /-enclosure`
    ...
    try {
      ...
      if (strict) {
        throw new Error(
          `bad regexp-value '${serializedValue}', parse error: ${e}`
        );
      } else {
        console.warn(
          `bad regexp-value '${serializedValue}', parse error: ${e}`
...
export class ContextKeyGreaterEqualsExpr implements IContextKeyExpression {
  ...
  public evaluate(context: IContext): boolean {
    return (
      parseFloat(<any>context.getValue(this.key)) >= parseFloat(this.value)
    );
  }
