...
import {
  ...
  PagedList,
  IPagedListOptions,
} from '../../../../vs/base/browser/ui/list/listPaging';
import {
...
} from '../../../../vs/base/browser/ui/tree/asyncDataTree';
import {
  DataTree,
  IDataTreeOptions,
} from '../../../../vs/base/browser/ui/tree/dataTree';
import {
...
import {
  ...
  ITableOptionsUpdate,
  Table,
} from '../../../../vs/base/browser/ui/table/tableWidget';
import {
  ITableColumn,
  ITableRenderer,
  ...
  register(
    ...
    if (!this._hasCreatedStyleController) {
      ...
      this.disposables.add(
        attachListStyler(styleController, this._themeService)
      );
    }
...
const RawWorkbenchListFocusContextKey = new RawContextKey<boolean>(
  'listFocus',
  true
);
...
export const WorkbenchListSupportsKeyboardNavigation =
  new RawContextKey<boolean>('listSupportsKeyboardNavigation', true);
export const WorkbenchListAutomaticKeyboardNavigationKey =
  'listAutomaticKeyboardNavigation';
export const WorkbenchListAutomaticKeyboardNavigation =
  new RawContextKey<boolean>(WorkbenchListAutomaticKeyboardNavigationKey, true);
...
const automaticKeyboardNavigationSettingKey =
...
function toWorkbenchListOptions<T>(
  ...
  if (
    ...
    const multipleSelectionController = new MultipleSelectionController(
  ...
  result.smoothScrolling = Boolean(
    configurationService.getValue<boolean>(listSmoothScrolling)
  );
  ...
  constructor(
    ...
    const horizontalScrolling =
      typeof options.horizontalScrolling !== 'undefined'
        ...
        : Boolean(
            configurationService.getValue<boolean>(horizontalScrollingKey)
          );
...
export class WorkbenchPagedList<T> extends PagedList<T> {
  ...
  constructor(
    ...
    if (options.overrideStyles) {
      this.disposables.add(
        attachListStyler(this, themeService, options.overrideStyles)
      );
    }
  ...
  constructor(
    ...
    if (
      ...
      this.openOnSingleClick =
        ...
        'doubleClick';
      this._register(
        options?.configurationService.onDidChangeConfiguration(() => {
          this.openOnSingleClick =
            options?.configurationService!.getValue(openModeSettingKey) !==
            'doubleClick';
        })
...
class TreeResourceNavigator<T, TFilterData> extends ResourceNavigator<T> {
  protected override readonly widget!:
...
>(
  ...
  const getAutomaticKeyboardNavigation = () => {
    ...
    let automaticKeyboardNavigation = Boolean(
  ...
  const keyboardNavigation =
    (options as any).simpleKeyboardNavigation || accessibilityOn
      ? 'simple'
      : configurationService.getValue<string>(keyboardNavigationSettingKey);
  const horizontalScrolling =
    (options as any).horizontalScrolling !== undefined
      ? (options as any).horizontalScrolling
      : Boolean(configurationService.getValue<boolean>(horizontalScrollingKey));
  const [workbenchListOptions, disposable] = toWorkbenchListOptions(
    options,
    configurationService,
    keybindingService
  );
  const additionalScrollHeight = (options as any).additionalScrollHeight;
  ...
  return {
    ...
    options: {
      ...
      renderIndentGuides: configurationService.getValue<RenderIndentGuides>(
...
class WorkbenchTreeInternals<TInput, T, TFilterData> {
  ...
  constructor(
    ...
    const listSupportsMultiSelect =
      WorkbenchListSupportsMultiSelectContextKey.bindTo(this.contextKeyService);
    listSupportsMultiSelect.set(
      !((options as any).multipleSelectionSupport === false)
    );
    ...
    const updateKeyboardNavigation = () => {
      const accessibilityOn = accessibilityService.isScreenReaderOptimized();
      const keyboardNavigation = accessibilityOn
        ? 'simple'
        : configurationService.getValue<string>(keyboardNavigationSettingKey);
      tree.updateOptions({
        simpleKeyboardNavigation: keyboardNavigation === 'simple',
    ...
    this.disposables.push(
      ...
      (tree as any).onDidChangeSelection(() => {
      ...
      configurationService.onDidChangeConfiguration((e) => {
        ...
        if (
          ...
          (options as any).expandOnlyOnTwistieClick === undefined
        ) {
          newOptions = {
            ...newOptions,
            expandOnlyOnTwistieClick:
        ...
        if (Object.keys(newOptions).length > 0) {
          (tree as any).updateOptions(newOptions);
        }
      }),
      this.contextKeyService.onDidChangeContext((e) => {
        if (e.affectsSome(interestingContextKeys)) {
          (tree as any).updateOptions({
            automaticKeyboardNavigation: getAutomaticKeyboardNavigation(),
          });
        }
      }),
  ...
  updateStyleOverrides(overrideStyles?: IColorMapping): void {
    dispose(this.styler);
    this.styler = overrideStyles
      ? attachListStyler(this.tree as any, this.themeService, overrideStyles)
      : Disposable.None;
  }
