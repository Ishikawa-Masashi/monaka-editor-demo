...
function fillInActions(
  ...
  for (const [group, actions] of groups) {
    ...
    for (let action of actions) {
      if (useAlternativeActions) {
        action =
          action instanceof MenuItemAction && action.alt ? action.alt : action;
      }
  ...
  for (const { group, action, index } of submenuInfo) {
    ...
    if (
      (submenuActions.length <= 1 ||
        target.length + submenuActions.length - 2 <= primaryMaxCount) &&
      shouldInlineSubmenu(action, group, target.length)
    ) {
  ...
  if (
    ...
    primaryBucket.length > primaryMaxCount
  ) {
    const overflow = primaryBucket.splice(
      primaryMaxCount,
      primaryBucket.length - primaryMaxCount
    );
...
export class MenuEntryActionViewItem extends ActionViewItem {
  ...
  override async onClick(event: MouseEvent): Promise<void> {
    ...
    try {
      ...
      this._notificationService.error(err as any);
    }
  }
  ...
  override render(container: HTMLElement): void {
    ...
    let alternativeKeyDown =
      ...
      ((isWindows || isLinux) && this._altKey.keyStatus.shiftKey);
    ...
    if (this._menuItemAction.alt) {
      this._register(
        this._altKey.event((value) => {
          alternativeKeyDown =
            value.altKey || ((isWindows || isLinux) && value.shiftKey);
          updateAltState();
        })
      );
    }
  ...
  override updateTooltip(): void {
    if (this.label) {
      ...
      if (!this._wantsAltCommand && this._menuItemAction.alt) {
        const altTooltip =
          this._menuItemAction.alt.tooltip || this._menuItemAction.alt.label;
  ...
  private _updateItemClass(item: ICommandAction): void {
    ...
    const icon =
      this._commandAction.checked && (item.toggled as { icon?: Icon })?.icon
        ? (item.toggled as { icon: Icon }).icon
        : item.icon;
...
export class SubmenuEntryActionViewItem extends DropdownMenuActionViewItem {
  constructor(
    ...
    super(action, { getActions: () => action.actions }, contextMenuService, {
      menuAsChild: true,
      classNames: ThemeIcon.isThemeIcon(action.item.icon)
...
export function createActionViewItem(
  ...
  action: IAction
): undefined | MenuEntryActionViewItem | SubmenuEntryActionViewItem {
