...
export class MenuService implements IMenuService {
  ...
  createMenu(
    ...
    emitEventsForSubmenuChanges: boolean = false
  ): IMenu {
    return new Menu(
      id,
...
class Menu implements IMenu {
  ...
  constructor(
    ...
    this._dispoables.add(
      _contextKeyService.onDidChangeContext((e) => {
  ...
  private _collectContextKeys(item: IMenuItem | ISubmenuItem): void {
    ...
    if (isIMenuItem(item)) {
      ...
      if (item.command.toggled) {
        const toggledExpression: any =
          (item.command.toggled as { condition: ContextKeyExpression })
