  ...
  implements IQuickAccessController
{
  ...
  private doShowOrPick(
    ...
    if (pick) {
      pickPromise = new Promise<IQuickPickItem[]>(
        (resolve) => (pickResolve = resolve)
      );
      disposables.add(
        once(picker.onWillAccept)((e) => {
          e.veto();
          picker.hide();
        })
      );
    ...
    // Register listeners
    disposables.add(
      this.registerPickerListeners(picker, provider, descriptor, value)
    );
    ...
    disposables.add(
      picker.onDidChangeValue((value) => {
        ...
        if (providerForValue !== provider) {
          this.show(
            value,
            { preserveValue: true } /* do not rewrite value from user typing! */
          );
        } else {
