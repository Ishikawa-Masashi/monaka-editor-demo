...
export class KeybindingResolver {
  ...
  constructor(
    ...
    for (let i = 0, len = this._defaultKeybindings.length; i < len; i++) {
      const command = this._defaultKeybindings[i].command;
      if (command) {
  ...
  private _addKeyPress(keypress: string, item: ResolvedKeybindingItem): void {
    ...
    for (let i = conflicts.length - 1; i >= 0; i--) {
      ...
      if (
        conflictIsChord &&
        itemIsChord &&
        conflict.keypressParts[1] !== item.keypressParts[1]
      ) {
  ...
  public lookupPrimaryKeybinding(
    ...
    const itemMatchingContext =
      context &&
      Array.from(items)
        .reverse()
        .find((item) => context.contextMatchesRules(item.when));
  ...
  public resolve(
    ...
    if (
      currentChord === null &&
      result.keypressParts.length > 1 &&
      result.keypressParts[1] !== null
    ) {
      this._log(
        `\\ From ${
          lookupMap.length
        } keybinding entries, matched chord, when: ${printWhenExplanation(
          result.when
